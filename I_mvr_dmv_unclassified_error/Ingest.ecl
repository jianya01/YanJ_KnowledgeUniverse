...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.record_id=RIGHT.record_id AND LEFT.lookup_text=RIGHT.lookup_text AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.error_source=RIGHT.error_source AND LEFT.transaction_date=RIGHT.transaction_date AND LEFT.state_abbr=RIGHT.state_abbr
              AND LEFT.content_data=RIGHT.content_data AND LEFT.date_added=RIGHT.date_added AND LEFT.user_added=RIGHT.user_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.record_id=RIGHT.record_id AND LEFT.lookup_text=RIGHT.lookup_text AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.error_source=RIGHT.error_source AND LEFT.transaction_date=RIGHT.transaction_date AND LEFT.state_abbr=RIGHT.state_abbr
              AND LEFT.content_data=RIGHT.content_data AND LEFT.date_added=RIGHT.date_added AND LEFT.user_added=RIGHT.user_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mvr_dmv_unclassified_error::Ingest_Cache',EXPIRE(I_mvr_dmv_unclassified_error.Config.PersistExpire));

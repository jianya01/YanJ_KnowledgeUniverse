...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.client_order_id=RIGHT.client_order_id AND LEFT.rules_product_id=RIGHT.rules_product_id AND LEFT.ruleplan_transaction_id=RIGHT.ruleplan_transaction_id AND LEFT.ruleplan_id=RIGHT.ruleplan_id AND LEFT.date_added=RIGHT.date_added
              AND LEFT.sequence=RIGHT.sequence AND LEFT.product_id=RIGHT.product_id AND LEFT.product_transaction_id=RIGHT.product_transaction_id AND LEFT.product_transaction_id_type=RIGHT.product_transaction_id_type AND LEFT.processing_status=RIGHT.processing_status AND LEFT.model_id=RIGHT.model_id AND LEFT.duplicate_report=RIGHT.duplicate_report AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.client_order_id=RIGHT.client_order_id AND LEFT.rules_product_id=RIGHT.rules_product_id AND LEFT.ruleplan_transaction_id=RIGHT.ruleplan_transaction_id AND LEFT.ruleplan_id=RIGHT.ruleplan_id AND LEFT.date_added=RIGHT.date_added
              AND LEFT.sequence=RIGHT.sequence AND LEFT.product_id=RIGHT.product_id AND LEFT.product_transaction_id=RIGHT.product_transaction_id AND LEFT.product_transaction_id_type=RIGHT.product_transaction_id_type AND LEFT.processing_status=RIGHT.processing_status AND LEFT.model_id=RIGHT.model_id AND LEFT.duplicate_report=RIGHT.duplicate_report AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_rules_transaction_log_detail::Ingest_Cache',EXPIRE(I_rules_transaction_log_detail.Config.PersistExpire));

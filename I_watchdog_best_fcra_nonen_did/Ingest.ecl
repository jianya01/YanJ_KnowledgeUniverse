...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_watchdog_best_fcra_nonen_did::Ingest_Cache',EXPIRE(I_watchdog_best_fcra_nonen_did.Config.PersistExpire));

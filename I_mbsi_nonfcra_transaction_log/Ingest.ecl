...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.login_id=RIGHT.login_id AND LEFT.billing_code=RIGHT.billing_code AND LEFT.function_name=RIGHT.function_name
              ...
              AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.price=RIGHT.price AND LEFT.currency=RIGHT.currency AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.record_count_1=RIGHT.record_count_1 AND LEFT.record_count_1_type=RIGHT.record_count_1_type AND LEFT.record_count_2=RIGHT.record_count_2 AND LEFT.record_count_2_type=RIGHT.record_count_2_type AND LEFT.result_format=RIGHT.result_format
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log.Config.PersistExpire));

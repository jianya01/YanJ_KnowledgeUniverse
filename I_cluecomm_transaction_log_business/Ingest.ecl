...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.business_name=RIGHT.business_name AND LEFT.tax_id=RIGHT.tax_id AND LEFT.phone=RIGHT.phone AND LEFT.experian_bin=RIGHT.experian_bin
              AND LEFT.austin_tetra=RIGHT.austin_tetra AND LEFT.duns_number=RIGHT.duns_number AND LEFT.date_added=RIGHT.date_added AND LEFT.potentialcorruption=RIGHT.potentialcorruption AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.business_name=RIGHT.business_name AND LEFT.tax_id=RIGHT.tax_id AND LEFT.phone=RIGHT.phone AND LEFT.experian_bin=RIGHT.experian_bin
              AND LEFT.austin_tetra=RIGHT.austin_tetra AND LEFT.duns_number=RIGHT.duns_number AND LEFT.date_added=RIGHT.date_added AND LEFT.potentialcorruption=RIGHT.potentialcorruption AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cluecomm_transaction_log_business::Ingest_Cache',EXPIRE(I_cluecomm_transaction_log_business.Config.PersistExpire));

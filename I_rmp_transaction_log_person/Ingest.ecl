...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.first_name=RIGHT.first_name AND LEFT.middle_name=RIGHT.middle_name AND LEFT.last_name=RIGHT.last_name AND LEFT.suffix_name=RIGHT.suffix_name
              ...
              AND LEFT.participant_policy_number=RIGHT.participant_policy_number AND LEFT.participant_carrier_name=RIGHT.participant_carrier_name AND LEFT.address_line=RIGHT.address_line AND LEFT.address_street_type=RIGHT.address_street_type AND LEFT.party_subj_id=RIGHT.party_subj_id AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_rmp_transaction_log_person::Ingest_Cache',EXPIRE(I_rmp_transaction_log_person.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.transaction_id_type=RIGHT.transaction_id_type AND LEFT.batch_source_name=RIGHT.batch_source_name AND LEFT.sequence_number=RIGHT.sequence_number AND LEFT.service_type=RIGHT.service_type AND LEFT.service_transaction_id=RIGHT.service_transaction_id
              AND LEFT.service_transaction_id_type=RIGHT.service_transaction_id_type AND LEFT.date_added=RIGHT.date_added AND LEFT.order_detail_id=RIGHT.order_detail_id AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.transaction_id_type=RIGHT.transaction_id_type AND LEFT.batch_source_name=RIGHT.batch_source_name AND LEFT.sequence_number=RIGHT.sequence_number AND LEFT.service_type=RIGHT.service_type AND LEFT.service_transaction_id=RIGHT.service_transaction_id
              AND LEFT.service_transaction_id_type=RIGHT.service_transaction_id_type AND LEFT.date_added=RIGHT.date_added AND LEFT.order_detail_id=RIGHT.order_detail_id AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mvr_transaction_log_detail::Ingest_Cache',EXPIRE(I_mvr_transaction_log_detail.Config.PersistExpire));

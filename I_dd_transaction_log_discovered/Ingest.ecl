...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.account_number=RIGHT.account_number AND LEFT.dd_lexid=RIGHT.dd_lexid AND LEFT.dd_inqstate=RIGHT.dd_inqstate AND LEFT.dd_datelastreported=RIGHT.dd_datelastreported
              AND LEFT.product_id=RIGHT.product_id AND LEFT.dd_last_name=RIGHT.dd_last_name AND LEFT.dd_first_name=RIGHT.dd_first_name AND LEFT.dd_dob=RIGHT.dd_dob AND LEFT.dd_dln=RIGHT.dd_dln AND LEFT.dd_dln_state=RIGHT.dd_dln_state AND LEFT.dd_ssn=RIGHT.dd_ssn AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.account_number=RIGHT.account_number AND LEFT.dd_lexid=RIGHT.dd_lexid AND LEFT.dd_inqstate=RIGHT.dd_inqstate AND LEFT.dd_datelastreported=RIGHT.dd_datelastreported
              AND LEFT.product_id=RIGHT.product_id AND LEFT.dd_last_name=RIGHT.dd_last_name AND LEFT.dd_first_name=RIGHT.dd_first_name AND LEFT.dd_dob=RIGHT.dd_dob AND LEFT.dd_dln=RIGHT.dd_dln AND LEFT.dd_dln_state=RIGHT.dd_dln_state AND LEFT.dd_ssn=RIGHT.dd_ssn AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_dd_transaction_log_discovered::Ingest_Cache',EXPIRE(I_dd_transaction_log_discovered.Config.PersistExpire));

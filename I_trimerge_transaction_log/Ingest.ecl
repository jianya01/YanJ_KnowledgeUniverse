...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.internal_account_id=RIGHT.internal_account_id AND LEFT.end_user=RIGHT.end_user AND LEFT.special_billing_id=RIGHT.special_billing_id
              AND LEFT.report_service_type=RIGHT.report_service_type AND LEFT.billing_report_code=RIGHT.billing_report_code AND LEFT.eqx_report_code=RIGHT.eqx_report_code AND LEFT.exp_report_code=RIGHT.exp_report_code AND LEFT.tran_report_code=RIGHT.tran_report_code AND LEFT.ln_report_code=RIGHT.ln_report_code AND LEFT.condition_code=RIGHT.condition_code AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.account_id=RIGHT.account_id
              AND LEFT.consumer_number=RIGHT.consumer_number AND LEFT.customer_number=RIGHT.customer_number AND LEFT.i_full_quote_back=RIGHT.i_full_quote_back AND LEFT.i_date_ordered=RIGHT.i_date_ordered AND LEFT.i_individual_ssn=RIGHT.i_individual_ssn AND LEFT.i_individual_name_prefix=RIGHT.i_individual_name_prefix AND LEFT.i_individual_last_name=RIGHT.i_individual_last_name AND LEFT.i_individual_first_name=RIGHT.i_individual_first_name AND LEFT.i_individual_middle_name=RIGHT.i_individual_middle_name AND LEFT.i_individual_name_suffix=RIGHT.i_individual_name_suffix
              AND LEFT.i_individual_dob_age=RIGHT.i_individual_dob_age AND LEFT.o_individual_ssn=RIGHT.o_individual_ssn AND LEFT.o_individual_vendor_ssn=RIGHT.o_individual_vendor_ssn AND LEFT.o_individual_name_prefix=RIGHT.o_individual_name_prefix AND LEFT.o_individual_last_name=RIGHT.o_individual_last_name AND LEFT.o_individual_first_name=RIGHT.o_individual_first_name AND LEFT.o_individual_middle_name=RIGHT.o_individual_middle_name AND LEFT.o_individual_name_suffix=RIGHT.o_individual_name_suffix AND LEFT.o_individual_dob_age=RIGHT.o_individual_dob_age AND LEFT.o_individual_processing_status=RIGHT.o_individual_processing_status
              AND LEFT.o_individual_error_code=RIGHT.o_individual_error_code AND LEFT.count_person_in=RIGHT.count_person_in AND LEFT.o_house_num_current_addr=RIGHT.o_house_num_current_addr AND LEFT.o_quad_current_addr=RIGHT.o_quad_current_addr AND LEFT.o_apt_current_addr=RIGHT.o_apt_current_addr AND LEFT.o_street_name_current_addr=RIGHT.o_street_name_current_addr AND LEFT.o_street_type_current_addr=RIGHT.o_street_type_current_addr AND LEFT.o_state_current_addr=RIGHT.o_state_current_addr AND LEFT.o_city_current_addr=RIGHT.o_city_current_addr AND LEFT.o_zip_current_addr=RIGHT.o_zip_current_addr
              AND LEFT.o_county_current_addr=RIGHT.o_county_current_addr AND LEFT.o_country_current_addr=RIGHT.o_country_current_addr AND LEFT.o_house_num_prior_addr=RIGHT.o_house_num_prior_addr AND LEFT.o_quad_prior_addr=RIGHT.o_quad_prior_addr AND LEFT.o_apt_prior_addr=RIGHT.o_apt_prior_addr AND LEFT.o_street_name_prior_addr=RIGHT.o_street_name_prior_addr AND LEFT.o_street_type_prior_addr=RIGHT.o_street_type_prior_addr AND LEFT.o_state_prior_addr=RIGHT.o_state_prior_addr AND LEFT.o_city_prior_addr=RIGHT.o_city_prior_addr AND LEFT.o_zip_prior_addr=RIGHT.o_zip_prior_addr
              AND LEFT.o_county_prior_addr=RIGHT.o_county_prior_addr AND LEFT.o_country_prior_addr=RIGHT.o_country_prior_addr AND LEFT.count_addr_in=RIGHT.count_addr_in AND LEFT.order_indicator=RIGHT.order_indicator AND LEFT.test_prod_indicator=RIGHT.test_prod_indicator AND LEFT.report_usage_code=RIGHT.report_usage_code AND LEFT.type_of_business=RIGHT.type_of_business AND LEFT.i_state_specific_account=RIGHT.i_state_specific_account AND LEFT.first_vendor=RIGHT.first_vendor AND LEFT.second_vendor=RIGHT.second_vendor
              AND LEFT.final_vendor=RIGHT.final_vendor AND LEFT.vendor_switch_condition=RIGHT.vendor_switch_condition AND LEFT.dupe_keys_selected=RIGHT.dupe_keys_selected AND LEFT.enhanced_dob=RIGHT.enhanced_dob AND LEFT.letter_indicator=RIGHT.letter_indicator AND LEFT.dupe_vendor_flag=RIGHT.dupe_vendor_flag AND LEFT.inquiry_single_order=RIGHT.inquiry_single_order AND LEFT.individual_beacon_score=RIGHT.individual_beacon_score AND LEFT.score_type_indicator=RIGHT.score_type_indicator AND LEFT.carrier_cnt=RIGHT.carrier_cnt
              ...
              AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_active=RIGHT.is_active AND LEFT.credit_public_records_verified=RIGHT.credit_public_records_verified AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.scoring_algo_name=RIGHT.scoring_algo_name AND LEFT.scoring_algo_score=RIGHT.scoring_algo_score AND LEFT.case_number=RIGHT.case_number
              AND LEFT.release_date=RIGHT.release_date AND LEFT.permissible_purpose=RIGHT.permissible_purpose AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.internal_account_id=RIGHT.internal_account_id AND LEFT.end_user=RIGHT.end_user AND LEFT.special_billing_id=RIGHT.special_billing_id
              AND LEFT.report_service_type=RIGHT.report_service_type AND LEFT.billing_report_code=RIGHT.billing_report_code AND LEFT.eqx_report_code=RIGHT.eqx_report_code AND LEFT.exp_report_code=RIGHT.exp_report_code AND LEFT.tran_report_code=RIGHT.tran_report_code AND LEFT.ln_report_code=RIGHT.ln_report_code AND LEFT.condition_code=RIGHT.condition_code AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.account_id=RIGHT.account_id
              AND LEFT.consumer_number=RIGHT.consumer_number AND LEFT.customer_number=RIGHT.customer_number AND LEFT.i_full_quote_back=RIGHT.i_full_quote_back AND LEFT.i_date_ordered=RIGHT.i_date_ordered AND LEFT.i_individual_ssn=RIGHT.i_individual_ssn AND LEFT.i_individual_name_prefix=RIGHT.i_individual_name_prefix AND LEFT.i_individual_last_name=RIGHT.i_individual_last_name AND LEFT.i_individual_first_name=RIGHT.i_individual_first_name AND LEFT.i_individual_middle_name=RIGHT.i_individual_middle_name AND LEFT.i_individual_name_suffix=RIGHT.i_individual_name_suffix
              AND LEFT.i_individual_dob_age=RIGHT.i_individual_dob_age AND LEFT.o_individual_ssn=RIGHT.o_individual_ssn AND LEFT.o_individual_vendor_ssn=RIGHT.o_individual_vendor_ssn AND LEFT.o_individual_name_prefix=RIGHT.o_individual_name_prefix AND LEFT.o_individual_last_name=RIGHT.o_individual_last_name AND LEFT.o_individual_first_name=RIGHT.o_individual_first_name AND LEFT.o_individual_middle_name=RIGHT.o_individual_middle_name AND LEFT.o_individual_name_suffix=RIGHT.o_individual_name_suffix AND LEFT.o_individual_dob_age=RIGHT.o_individual_dob_age AND LEFT.o_individual_processing_status=RIGHT.o_individual_processing_status
              AND LEFT.o_individual_error_code=RIGHT.o_individual_error_code AND LEFT.count_person_in=RIGHT.count_person_in AND LEFT.o_house_num_current_addr=RIGHT.o_house_num_current_addr AND LEFT.o_quad_current_addr=RIGHT.o_quad_current_addr AND LEFT.o_apt_current_addr=RIGHT.o_apt_current_addr AND LEFT.o_street_name_current_addr=RIGHT.o_street_name_current_addr AND LEFT.o_street_type_current_addr=RIGHT.o_street_type_current_addr AND LEFT.o_state_current_addr=RIGHT.o_state_current_addr AND LEFT.o_city_current_addr=RIGHT.o_city_current_addr AND LEFT.o_zip_current_addr=RIGHT.o_zip_current_addr
              ...
              AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.product_line=RIGHT.product_line AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method
              AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_active=RIGHT.is_active AND LEFT.credit_public_records_verified=RIGHT.credit_public_records_verified AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.scoring_algo_name=RIGHT.scoring_algo_name AND LEFT.scoring_algo_score=RIGHT.scoring_algo_score AND LEFT.case_number=RIGHT.case_number
              AND LEFT.release_date=RIGHT.release_date AND LEFT.permissible_purpose=RIGHT.permissible_purpose AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_trimerge_transaction_log::Ingest_Cache',EXPIRE(I_trimerge_transaction_log.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.product_id=RIGHT.product_id AND LEFT.prior_policy_number=RIGHT.prior_policy_number AND LEFT.policy_effective_date=RIGHT.policy_effective_date AND LEFT.date_added=RIGHT.date_added
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cluecomm_transaction_log_priorpolicyno::Ingest_Cache',EXPIRE(I_cluecomm_transaction_log_priorpolicyno.Config.PersistExpire));

...
//FirstDegreeRelativesAndAssociates := KELGlobal.Q_First_Degree_Relatives_And_Associates(LexID).Res0;
...
BocaShellSnapshots			:= JOIN(LexIDSet, KELGlobal.Q_All_Boca_Shell_Snapshots(HistoricalDate).Res0, LEFT.LexID = RIGHT.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);
InsuranceHeaderSnapshots	:= JOIN(LexIDSet, KELGlobal.Q_All_Insurance_Header_Snapshots(HistoricalDate).Res0, LEFT.LexID = RIGHT.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);
LengthOfResidencySnapshots	:= JOIN(LexIDSet, KELGlobal.Q_All_Length_Of_Residency_Snapshots(HistoricalDate).Res0, LEFT.LexID = RIGHT.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);
MaritalStatusSnapshots		:= JOIN(LexIDSet, KELGlobal.Q_All_Marital_Status_Snapshots(HistoricalDate).Res0, LEFT.LexID = RIGHT.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);
MarketViewSnapshots			:= JOIN(LexIDSet, KELGlobal.Q_All_Market_View_Snapshots(HistoricalDate).Res0, LEFT.LexID = RIGHT.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);
RiskViewSnapshots			:= JOIN(LexIDSet, KELGlobal.Q_All_Risk_View_Snapshots(HistoricalDate).Res0, LEFT.LexID = RIGHT.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);
#end

// This query produces the Relatives and Associates for a given set of LexID's
//FirstDegreeRelativesAndAssociates := JOIN(LexIDSet, KELGlobal.Q_All_First_Degree_Relatives_And_Associates.Res0, LEFT.LexID = RIGHT.Subject_.UID, TRANSFORM(RIGHT), KEEP(1), ATMOST(10), SMART);

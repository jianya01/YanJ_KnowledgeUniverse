...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_vin_transaction_log_extension::Ingest_Cache',EXPIRE(I_vin_transaction_log_extension.Config.PersistExpire));

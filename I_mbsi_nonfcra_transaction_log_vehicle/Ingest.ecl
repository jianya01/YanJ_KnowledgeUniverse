...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.order_date=RIGHT.order_date AND LEFT.vin=RIGHT.vin
              AND LEFT.model_year=RIGHT.model_year AND LEFT.make_model=RIGHT.make_model AND LEFT.plate_data=RIGHT.plate_data AND LEFT.plate_state=RIGHT.plate_state AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.order_date=RIGHT.order_date AND LEFT.vin=RIGHT.vin
              AND LEFT.model_year=RIGHT.model_year AND LEFT.make_model=RIGHT.make_model AND LEFT.plate_data=RIGHT.plate_data AND LEFT.plate_state=RIGHT.plate_state AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_vehicle::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_vehicle.Config.PersistExpire));

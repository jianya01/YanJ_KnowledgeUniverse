...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.client_order_id=RIGHT.client_order_id AND LEFT.rules_product_id=RIGHT.rules_product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.switch_fee_indicator=RIGHT.switch_fee_indicator AND LEFT.login_id=RIGHT.login_id
              AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.function_name=RIGHT.function_name AND LEFT.report_code=RIGHT.report_code AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.vendor_account_base=RIGHT.vendor_account_base AND LEFT.vendor_account_suffix=RIGHT.vendor_account_suffix AND LEFT.account_id=RIGHT.account_id AND LEFT.customer_number=RIGHT.customer_number AND LEFT.full_quote_back=RIGHT.full_quote_back
              AND LEFT.date_ordered=RIGHT.date_ordered AND LEFT.unique_id=RIGHT.unique_id AND LEFT.unique_id_type=RIGHT.unique_id_type AND LEFT.record_version=RIGHT.record_version AND LEFT.reference_number=RIGHT.reference_number AND LEFT.line_of_business=RIGHT.line_of_business AND LEFT.processing_status=RIGHT.processing_status AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.price=RIGHT.price AND LEFT.currency=RIGHT.currency
              AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.result_format=RIGHT.result_format AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.product_line=RIGHT.product_line AND LEFT.response_time=RIGHT.response_time
              AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.esp_method=RIGHT.esp_method AND LEFT.account_type=RIGHT.account_type AND LEFT.bill_no_hit=RIGHT.bill_no_hit
              AND LEFT.glb_purpose=RIGHT.glb_purpose AND LEFT.dppa_purpose=RIGHT.dppa_purpose AND LEFT.permissible_purpose=RIGHT.permissible_purpose AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.client_order_id=RIGHT.client_order_id AND LEFT.rules_product_id=RIGHT.rules_product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.switch_fee_indicator=RIGHT.switch_fee_indicator AND LEFT.login_id=RIGHT.login_id
              AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.function_name=RIGHT.function_name AND LEFT.report_code=RIGHT.report_code AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.vendor_account_base=RIGHT.vendor_account_base AND LEFT.vendor_account_suffix=RIGHT.vendor_account_suffix AND LEFT.account_id=RIGHT.account_id AND LEFT.customer_number=RIGHT.customer_number AND LEFT.full_quote_back=RIGHT.full_quote_back
              AND LEFT.date_ordered=RIGHT.date_ordered AND LEFT.unique_id=RIGHT.unique_id AND LEFT.unique_id_type=RIGHT.unique_id_type AND LEFT.record_version=RIGHT.record_version AND LEFT.reference_number=RIGHT.reference_number AND LEFT.line_of_business=RIGHT.line_of_business AND LEFT.processing_status=RIGHT.processing_status AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.price=RIGHT.price AND LEFT.currency=RIGHT.currency
              AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.result_format=RIGHT.result_format AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.product_line=RIGHT.product_line AND LEFT.response_time=RIGHT.response_time
              AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.esp_method=RIGHT.esp_method AND LEFT.account_type=RIGHT.account_type AND LEFT.bill_no_hit=RIGHT.bill_no_hit
              AND LEFT.glb_purpose=RIGHT.glb_purpose AND LEFT.dppa_purpose=RIGHT.dppa_purpose AND LEFT.permissible_purpose=RIGHT.permissible_purpose AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_rules_transaction_log::Ingest_Cache',EXPIRE(I_rules_transaction_log.Config.PersistExpire));

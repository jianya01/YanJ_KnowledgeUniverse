...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.policy_sequence=RIGHT.policy_sequence AND LEFT.sequence=RIGHT.sequence AND LEFT.deductible_overall=RIGHT.deductible_overall AND LEFT.deductible_theft=RIGHT.deductible_theft AND LEFT.street_number=RIGHT.street_number
              AND LEFT.street_name=RIGHT.street_name AND LEFT.apartment=RIGHT.apartment AND LEFT.state=RIGHT.state AND LEFT.zip=RIGHT.zip AND LEFT.zip_4=RIGHT.zip_4 AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.policy_sequence=RIGHT.policy_sequence AND LEFT.sequence=RIGHT.sequence AND LEFT.deductible_overall=RIGHT.deductible_overall AND LEFT.deductible_theft=RIGHT.deductible_theft AND LEFT.street_number=RIGHT.street_number
              AND LEFT.street_name=RIGHT.street_name AND LEFT.apartment=RIGHT.apartment AND LEFT.state=RIGHT.state AND LEFT.zip=RIGHT.zip AND LEFT.zip_4=RIGHT.zip_4 AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cc_transaction_log_policy_property::Ingest_Cache',EXPIRE(I_cc_transaction_log_policy_property.Config.PersistExpire));

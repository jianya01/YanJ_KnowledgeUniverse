...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.verification_id=RIGHT.verification_id AND LEFT.defendant_name=RIGHT.defendant_name AND LEFT.defendant_address=RIGHT.defendant_address AND LEFT.defendant_ssn=RIGHT.defendant_ssn AND LEFT.plaintiff=RIGHT.plaintiff AND LEFT.case_number=RIGHT.case_number
              AND LEFT.judgement_type=RIGHT.judgement_type AND LEFT.other_judgement_type=RIGHT.other_judgement_type AND LEFT.judgement_date=RIGHT.judgement_date AND LEFT.amount=RIGHT.amount AND LEFT.disposition=RIGHT.disposition AND LEFT.case_status=RIGHT.case_status AND LEFT.satisfied_date=RIGHT.satisfied_date AND LEFT.satisfaction_document_number=RIGHT.satisfaction_document_number AND LEFT.date_added=RIGHT.date_added AND LEFT.user_added=RIGHT.user_added
              AND LEFT.date_changed=RIGHT.date_changed AND LEFT.user_changed=RIGHT.user_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.verification_id=RIGHT.verification_id AND LEFT.defendant_name=RIGHT.defendant_name AND LEFT.defendant_address=RIGHT.defendant_address AND LEFT.defendant_ssn=RIGHT.defendant_ssn AND LEFT.plaintiff=RIGHT.plaintiff AND LEFT.case_number=RIGHT.case_number
              AND LEFT.judgement_type=RIGHT.judgement_type AND LEFT.other_judgement_type=RIGHT.other_judgement_type AND LEFT.judgement_date=RIGHT.judgement_date AND LEFT.amount=RIGHT.amount AND LEFT.disposition=RIGHT.disposition AND LEFT.case_status=RIGHT.case_status AND LEFT.satisfied_date=RIGHT.satisfied_date AND LEFT.satisfaction_document_number=RIGHT.satisfaction_document_number AND LEFT.date_added=RIGHT.date_added AND LEFT.user_added=RIGHT.user_added
              AND LEFT.date_changed=RIGHT.date_changed AND LEFT.user_changed=RIGHT.user_changed AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_trimerge_trimerge_caopr_verification_judgement::Ingest_Cache',EXPIRE(I_trimerge_trimerge_caopr_verification_judgement.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.service_type=RIGHT.service_type AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.report_code=RIGHT.report_code
              AND LEFT.customer_number=RIGHT.customer_number AND LEFT.order_point_company_number=RIGHT.order_point_company_number AND LEFT.full_quote_back=RIGHT.full_quote_back AND LEFT.i_date_ordered=RIGHT.i_date_ordered AND LEFT.count_person_in=RIGHT.count_person_in AND LEFT.count_person_used=RIGHT.count_person_used AND LEFT.i_addr1_house_num=RIGHT.i_addr1_house_num AND LEFT.i_addr1_apt_num=RIGHT.i_addr1_apt_num AND LEFT.i_addr1_line=RIGHT.i_addr1_line AND LEFT.i_addr1_state=RIGHT.i_addr1_state
              AND LEFT.i_addr1_zip=RIGHT.i_addr1_zip AND LEFT.i_addr1_city=RIGHT.i_addr1_city AND LEFT.i_addr1_county=RIGHT.i_addr1_county AND LEFT.i_addr1_country=RIGHT.i_addr1_country AND LEFT.i_addr1_type=RIGHT.i_addr1_type AND LEFT.i_addr2_house_num=RIGHT.i_addr2_house_num AND LEFT.i_addr2_apt_num=RIGHT.i_addr2_apt_num AND LEFT.i_addr2_line=RIGHT.i_addr2_line AND LEFT.i_addr2_state=RIGHT.i_addr2_state AND LEFT.i_addr2_zip=RIGHT.i_addr2_zip
              AND LEFT.i_addr2_city=RIGHT.i_addr2_city AND LEFT.i_addr2_county=RIGHT.i_addr2_county AND LEFT.i_addr2_country=RIGHT.i_addr2_country AND LEFT.i_addr2_type=RIGHT.i_addr2_type AND LEFT.i_addr3_line=RIGHT.i_addr3_line AND LEFT.i_addr3_house_num=RIGHT.i_addr3_house_num AND LEFT.i_addr3_apt_num=RIGHT.i_addr3_apt_num AND LEFT.i_addr3_state=RIGHT.i_addr3_state AND LEFT.i_addr3_zip=RIGHT.i_addr3_zip AND LEFT.i_addr3_city=RIGHT.i_addr3_city
              AND LEFT.i_addr3_county=RIGHT.i_addr3_county AND LEFT.i_addr3_country=RIGHT.i_addr3_country AND LEFT.i_addr3_type=RIGHT.i_addr3_type AND LEFT.count_addr_in=RIGHT.count_addr_in AND LEFT.count_addr_used=RIGHT.count_addr_used AND LEFT.count_vehicle_in=RIGHT.count_vehicle_in AND LEFT.count_vehicle_used=RIGHT.count_vehicle_used AND LEFT.vin_d_code=RIGHT.vin_d_code AND LEFT.jua_ltr_code=RIGHT.jua_ltr_code AND LEFT.prc_founds=RIGHT.prc_founds
              AND LEFT.addit_driver_reported=RIGHT.addit_driver_reported AND LEFT.prior_inquiry_found=RIGHT.prior_inquiry_found AND LEFT.dd_flag=RIGHT.dd_flag AND LEFT.policy_number=RIGHT.policy_number AND LEFT.inq_hist_claims=RIGHT.inq_hist_claims AND LEFT.count_claims_found=RIGHT.count_claims_found AND LEFT.count_claims_reported=RIGHT.count_claims_reported AND LEFT.requestor_type=RIGHT.requestor_type AND LEFT.test_prod_indicator=RIGHT.test_prod_indicator AND LEFT.iso_vm=RIGHT.iso_vm
              AND LEFT.line_id=RIGHT.line_id AND LEFT.inquiry_history_records_added=RIGHT.inquiry_history_records_added AND LEFT.processing_comments=RIGHT.processing_comments AND LEFT.record_version=RIGHT.record_version AND LEFT.reference_number=RIGHT.reference_number AND LEFT.processing_status=RIGHT.processing_status AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.price=RIGHT.price AND LEFT.currency=RIGHT.currency AND LEFT.pricing_error_code=RIGHT.pricing_error_code
              AND LEFT.free=RIGHT.free AND LEFT.record_count=RIGHT.record_count AND LEFT.result_format=RIGHT.result_format AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.product_line=RIGHT.product_line AND LEFT.login_history_id=RIGHT.login_history_id
              AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_active=RIGHT.is_active AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_CDSC_transaction_log::Ingest_Cache',EXPIRE(I_CDSC_transaction_log.Config.PersistExpire));

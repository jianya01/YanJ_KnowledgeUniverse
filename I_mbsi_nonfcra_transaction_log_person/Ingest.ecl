...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_person::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_person.Config.PersistExpire));

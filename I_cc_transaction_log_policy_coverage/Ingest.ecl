...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.policy_sequence=RIGHT.policy_sequence AND LEFT.vehicle_or_property_sequence=RIGHT.vehicle_or_property_sequence AND LEFT.sequence=RIGHT.sequence AND LEFT.coverage_type_indicator=RIGHT.coverage_type_indicator AND LEFT.coverage_type=RIGHT.coverage_type
              AND LEFT.individual_limit=RIGHT.individual_limit AND LEFT.ocurrence_limit=RIGHT.ocurrence_limit AND LEFT.combined_single_limit=RIGHT.combined_single_limit AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.policy_sequence=RIGHT.policy_sequence AND LEFT.vehicle_or_property_sequence=RIGHT.vehicle_or_property_sequence AND LEFT.sequence=RIGHT.sequence AND LEFT.coverage_type_indicator=RIGHT.coverage_type_indicator AND LEFT.coverage_type=RIGHT.coverage_type
              AND LEFT.individual_limit=RIGHT.individual_limit AND LEFT.ocurrence_limit=RIGHT.ocurrence_limit AND LEFT.combined_single_limit=RIGHT.combined_single_limit AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cc_transaction_log_policy_coverage::Ingest_Cache',EXPIRE(I_cc_transaction_log_policy_coverage.Config.PersistExpire));

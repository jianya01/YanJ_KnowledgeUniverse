...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.first_name=RIGHT.first_name AND LEFT.middle_name=RIGHT.middle_name AND LEFT.last_name=RIGHT.last_name AND LEFT.suffix_name=RIGHT.suffix_name
              AND LEFT.ssn=RIGHT.ssn AND LEFT.current_dl_state=RIGHT.current_dl_state AND LEFT.current_dl_number=RIGHT.current_dl_number AND LEFT.prior_dl_state=RIGHT.prior_dl_state AND LEFT.prior_dl_number=RIGHT.prior_dl_number AND LEFT.prior_policy_number=RIGHT.prior_policy_number AND LEFT.date_added=RIGHT.date_added AND LEFT.reference_no=RIGHT.reference_no AND LEFT.unit_no=RIGHT.unit_no AND LEFT.customer_no=RIGHT.customer_no
              AND LEFT.quoteback=RIGHT.quoteback AND LEFT.order_date=RIGHT.order_date AND LEFT.current_house_no=RIGHT.current_house_no AND LEFT.current_street_name=RIGHT.current_street_name AND LEFT.current_apt_no=RIGHT.current_apt_no AND LEFT.current_city=RIGHT.current_city AND LEFT.current_state=RIGHT.current_state AND LEFT.current_zip=RIGHT.current_zip AND LEFT.current_zip_ext=RIGHT.current_zip_ext AND LEFT.former_house_no=RIGHT.former_house_no
              AND LEFT.former_street_name=RIGHT.former_street_name AND LEFT.former_apt_no=RIGHT.former_apt_no AND LEFT.former_city=RIGHT.former_city AND LEFT.former_state=RIGHT.former_state AND LEFT.former_zip=RIGHT.former_zip AND LEFT.former_zip_ext=RIGHT.former_zip_ext AND LEFT.dob=RIGHT.dob AND LEFT.sex=RIGHT.sex AND LEFT.service_type=RIGHT.service_type AND LEFT.spl_bill_id=RIGHT.spl_bill_id
              AND LEFT.product_id=RIGHT.product_id AND LEFT.telephone_area_code=RIGHT.telephone_area_code AND LEFT.telephone_exchange=RIGHT.telephone_exchange AND LEFT.telephone_number=RIGHT.telephone_number AND LEFT.subject_process_stat_code=RIGHT.subject_process_stat_code AND LEFT.mortgage_no=RIGHT.mortgage_no AND LEFT.lex_id=RIGHT.lex_id AND LEFT.potentialcorruption=RIGHT.potentialcorruption AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.first_name=RIGHT.first_name AND LEFT.middle_name=RIGHT.middle_name AND LEFT.last_name=RIGHT.last_name AND LEFT.suffix_name=RIGHT.suffix_name
              ...
              AND LEFT.quoteback=RIGHT.quoteback AND LEFT.order_date=RIGHT.order_date AND LEFT.current_house_no=RIGHT.current_house_no AND LEFT.current_street_name=RIGHT.current_street_name AND LEFT.current_apt_no=RIGHT.current_apt_no AND LEFT.current_city=RIGHT.current_city AND LEFT.current_state=RIGHT.current_state AND LEFT.current_zip=RIGHT.current_zip AND LEFT.current_zip_ext=RIGHT.current_zip_ext AND LEFT.former_house_no=RIGHT.former_house_no
              AND LEFT.former_street_name=RIGHT.former_street_name AND LEFT.former_apt_no=RIGHT.former_apt_no AND LEFT.former_city=RIGHT.former_city AND LEFT.former_state=RIGHT.former_state AND LEFT.former_zip=RIGHT.former_zip AND LEFT.former_zip_ext=RIGHT.former_zip_ext AND LEFT.dob=RIGHT.dob AND LEFT.sex=RIGHT.sex AND LEFT.service_type=RIGHT.service_type AND LEFT.spl_bill_id=RIGHT.spl_bill_id
              AND LEFT.product_id=RIGHT.product_id AND LEFT.telephone_area_code=RIGHT.telephone_area_code AND LEFT.telephone_exchange=RIGHT.telephone_exchange AND LEFT.telephone_number=RIGHT.telephone_number AND LEFT.subject_process_stat_code=RIGHT.subject_process_stat_code AND LEFT.mortgage_no=RIGHT.mortgage_no AND LEFT.lex_id=RIGHT.lex_id AND LEFT.potentialcorruption=RIGHT.potentialcorruption AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_clue_transaction_log_person::Ingest_Cache',EXPIRE(I_clue_transaction_log_person.Config.PersistExpire));

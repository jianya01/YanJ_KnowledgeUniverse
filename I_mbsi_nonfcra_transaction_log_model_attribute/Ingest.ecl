...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.attribute_sequence=RIGHT.attribute_sequence AND LEFT.model_sequence=RIGHT.model_sequence AND LEFT.person_sequence=RIGHT.person_sequence AND LEFT.processing_status=RIGHT.processing_status AND LEFT.attribute_source=RIGHT.attribute_source
              AND LEFT.attribute_name=RIGHT.attribute_name AND LEFT.attribute_value=RIGHT.attribute_value AND LEFT.lex_id=RIGHT.lex_id AND LEFT.model_id=RIGHT.model_id AND LEFT.claim_number=RIGHT.claim_number AND LEFT.ambest_number=RIGHT.ambest_number AND LEFT.policy_number=RIGHT.policy_number AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.attribute_sequence=RIGHT.attribute_sequence AND LEFT.model_sequence=RIGHT.model_sequence AND LEFT.person_sequence=RIGHT.person_sequence AND LEFT.processing_status=RIGHT.processing_status AND LEFT.attribute_source=RIGHT.attribute_source
              AND LEFT.attribute_name=RIGHT.attribute_name AND LEFT.attribute_value=RIGHT.attribute_value AND LEFT.lex_id=RIGHT.lex_id AND LEFT.model_id=RIGHT.model_id AND LEFT.claim_number=RIGHT.claim_number AND LEFT.ambest_number=RIGHT.ambest_number AND LEFT.policy_number=RIGHT.policy_number AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_model_attribute::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_model_attribute.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.person_seq=RIGHT.person_seq AND LEFT.license_seq=RIGHT.license_seq AND LEFT.dl_classification=RIGHT.dl_classification AND LEFT.dl_state=RIGHT.dl_state
              AND LEFT.dl_number=RIGHT.dl_number AND LEFT.dl_class=RIGHT.dl_class AND LEFT.dl_dob=RIGHT.dl_dob AND LEFT.dl_exp=RIGHT.dl_exp AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.person_seq=RIGHT.person_seq AND LEFT.license_seq=RIGHT.license_seq AND LEFT.dl_classification=RIGHT.dl_classification AND LEFT.dl_state=RIGHT.dl_state
              AND LEFT.dl_number=RIGHT.dl_number AND LEFT.dl_class=RIGHT.dl_class AND LEFT.dl_dob=RIGHT.dl_dob AND LEFT.dl_exp=RIGHT.dl_exp AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_license::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_license.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.business_name=RIGHT.business_name AND LEFT.dba_name=RIGHT.dba_name AND LEFT.fein=RIGHT.fein AND LEFT.line_of_business=RIGHT.line_of_business
              AND LEFT.principal_name=RIGHT.principal_name AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.business_name=RIGHT.business_name AND LEFT.dba_name=RIGHT.dba_name AND LEFT.fein=RIGHT.fein AND LEFT.line_of_business=RIGHT.line_of_business
              AND LEFT.principal_name=RIGHT.principal_name AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_business::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_business.Config.PersistExpire));

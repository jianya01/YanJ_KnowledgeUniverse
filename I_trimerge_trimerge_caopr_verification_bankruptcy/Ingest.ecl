...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.verification_id=RIGHT.verification_id AND LEFT.debtor_name=RIGHT.debtor_name AND LEFT.debtor_address=RIGHT.debtor_address AND LEFT.debtor_ssn=RIGHT.debtor_ssn AND LEFT.bankruptcy_petition_number=RIGHT.bankruptcy_petition_number AND LEFT.file_date=RIGHT.file_date
              AND LEFT.bankruptcy_type=RIGHT.bankruptcy_type AND LEFT.is_voluntary=RIGHT.is_voluntary AND LEFT.status=RIGHT.status AND LEFT.released_date=RIGHT.released_date AND LEFT.released_document_number=RIGHT.released_document_number AND LEFT.date_added=RIGHT.date_added AND LEFT.user_added=RIGHT.user_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.user_changed=RIGHT.user_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.verification_id=RIGHT.verification_id AND LEFT.debtor_name=RIGHT.debtor_name AND LEFT.debtor_address=RIGHT.debtor_address AND LEFT.debtor_ssn=RIGHT.debtor_ssn AND LEFT.bankruptcy_petition_number=RIGHT.bankruptcy_petition_number AND LEFT.file_date=RIGHT.file_date
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_trimerge_trimerge_caopr_verification_bankruptcy::Ingest_Cache',EXPIRE(I_trimerge_trimerge_caopr_verification_bankruptcy.Config.PersistExpire));

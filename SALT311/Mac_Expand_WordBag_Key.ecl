...
export mac_expand_wordbag_key(inkey,hash_field,field,outkey,scorefield='') := MACRO
  #uniquename(tr)
	typeof(inkey) %tr%(inkey le,unsigned c) := transform
	  self.hash_field := hash32(SALT311.GetNthWord(le.field,c));
		#IF (#TEXT(scorefield)<>'')
		self.scorefield := (unsigned)SALT311.GetNthWord(le.field,c+1);
		#END
	  self := le;
	end;
	outkey := normalize(inkey,SALT311.WordCount(left.field)/2,%tr%(left,counter*2));

...
//using namespace std;
namespace ns_SALT311_Fn_Match_Bagofwords_Multiway {
...
using namespace ns_SALT311_Fn_Match_Bagofwords_Multiway;
#body
#option pure
{
    ...
    for (unsigned i=0; i<cmd.number(); i++)
    	if (cmd[i]->stem())
    		stem = true;	
 		ns_SALT311_Fn_Match_Bagofwords_Multiway::wb L(lenL,l,stem);
    ns_SALT311_Fn_Match_Bagofwords_Multiway::wb R(lenR,r,stem);
    if ( L.error || R.error )
    ...
    // Short & Long
    ns_SALT311_Fn_Match_Bagofwords_Multiway::wb SH = L.nwords < R.nwords || lenL < lenR ? L : R;
    ns_SALT311_Fn_Match_Bagofwords_Multiway::wb LO = L.nwords < R.nwords || lenL < lenR ? R : L;

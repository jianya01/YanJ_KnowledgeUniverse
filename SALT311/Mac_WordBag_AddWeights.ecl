export mac_wordbag_addweights(infile,infield,value_file,outfile) := MACRO
  ...
  #uniquename(r)
	...
	%r% %nt%(%p% le,unsigned c) := transform
		self.word := SALT311.GetNthWord(le.infield,c);
		self.pos := c;
		self := le;
	  end;
	#uniquename(n)
	%n% := normalize(%p%,SALT311.WordCount(left.infield),%nt%(left,counter));
	#uniquename(add_s)
	%r% %add_s%(%n% le,value_file ri) := transform
	  self.word := trim(le.word) + ' ' + (SALT311.StrType)((unsigned)ri.field_specificity);
	  self := le;
	  end;
	#uniquename(j)

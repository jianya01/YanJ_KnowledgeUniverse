...
NB(SALT311.StrType s) := SALT311.StringFilterOut(s,' -');
...
Fix(SALT311.StrType s) := SALT311.StringCleanSpaces(SALT311.StringSubstituteOut(s,'-',' '));
TYPEOF(fieldvals) apfw(fieldvals le,UNSIGNED c) := TRANSFORM
  SELF.infield := SALT311.GetNthWord(Fix(le.infield),c);
  SELF := le;
  END;
sw0 := DISTRIBUTED(NORMALIZE(fieldvals,SALT311.WordCount(Fix(LEFT.infield)),apfw(LEFT,COUNTER)),ufield); // Get each word

...
export Data_Layout := RECORD
  SALT311.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
export ResultLine_Layout := record(Field_Identification)
	...
	SALT311.Str30Type 					MinVal30;
	SALT311.Str30Type 					MaxVal30;
...
EXPORT EclRecord(DATASET(ResultLine_Layout) d, STRING RName='Record_Layout') := FUNCTION
	...
	r1 WriteEcl(t le) := TRANSFORM
		...
		SELF.Txt := '  '+MAP( ~le.AllNum => 'UNICODE' + IF(le.MaxLen<MaxFixedLen,(SALT311.StrType)le.MaxLen, ''),
#else
		SELF.Txt := '  '+MAP( ~le.AllNum => 'STRING' + IF(le.MaxLen<MaxFixedLen,(SALT311.StrType)le.MaxLen, ''),
#end
...
EXPORT FieldTypes(DATASET(ResultLine_Layout) d, REAL Recall = 99.9) := FUNCTION
  PruneOutliers(file,cnt_field) := FUNCTIONMACRO
	...
	ENDMACRO;
  SALT311.StrType GetChars(DATASET(Character_Layout) gc) := FUNCTION
	  T := TABLE(SORT(PruneOutliers(gc(),cnt),c),{ SALT311.StrType s := c; });
		RETURN ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s;
	END;
	
	SALT311.StrType LenList(DATASET(Length_Layout) lens) := FUNCTION
	  T := TABLE(PruneOutliers(lens,cnt),{ SALT311.StrType s := ','+len; });
		s := ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s[2..];
	  RETURN ':LENGTHS('+s+')';
	END;
	SALT311.StrType WordList(DATASET(Words_Layout) wds) := FUNCTION
	  T := TABLE(PruneOutliers(wds,cnt),{ SALT311.StrType s := ','+words; });
		s := ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s[2..];
	  RETURN ':WORDS('+s+')';
	END;
	...
	R := RECORD
	  d.FldNo;
	  d.FieldName;
		SALT311.StrType Allow := ':ALLOW('+GetChars(d.Characters)+')';
		SALT311.StrType LTrim := IF ( EXISTS(d.patterns(data_pattern[1]=' ',TRIM(data_pattern[2..])<>'')),'',':LEFTTRIM');
		SALT311.StrType Lens := LenList(d.len);
		SALT311.StrType Words := WordList(d.words);
		SALT311.StrType Spaces := ':SPACES( )';
	END;
	t := TABLE(d,r);
	r1 := RECORD
	  t.FldNo;
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT311.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT311.WordCount((SALT311.StrType)FldInv1.Fld);
  end;
...
card_rec := RECORD
  FldInv.FldNo;
	...
	SALT311.Str30Type MaxVal30 := MAX(GROUP,FldInv.Fld);
	SALT311.Str30Type MinVal30 := MIN(GROUP,IF(FldInv.Fld='','ZZ--BOGUSVALUE--ZZ',FldInv.Fld));
  END;	
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT311.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT311.StrType Fld  {maxlength(200000)} := SALT311.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

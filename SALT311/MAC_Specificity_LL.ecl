...
%temp_georec_pairs% := record
  SALT311.Str20Type infield;
	...
	%all_join% := join(%g2%,%g2%,
		...
		transform(%temp_georec_pairs%,
		  self.infield := left.infield;
			...
			self.dist := truncate(SALT311.utLL_Dist(left.lat,left.long,right.lat,right.long)+.5),
			self.cnt := right.cnt,
	...
	%ff% := record,maxlength(2048)
	  SALT311.Str20Type infield;
		dataset(SALT311.Layout_LL_Ranges) ranges;
		end;
	// finally, dedup to eliminate all but the last instance of each specificity
	#uniquename(o)
	%o% := project(dedup(%i%,lat1,long1,specificity,right,local),transform(%ff%,self.ranges := dataset([{left.dist,left.specificity}],SALT311.layout_ll_ranges),self.infield := left.infield));
	#uniquename(o2)

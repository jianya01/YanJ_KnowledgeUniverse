// NOTE: This macro references SALT311.Config.LookupMaxSize when determining whether
...
t := TABLE(infile,ifp);
SALT311.utMAC_Sequence_Records(t,xxx_unc,nif)	
r := RECORD
  UNSIGNED xxx_unc;
  UNSIGNED2 pos;
  SALT311.StrType s;
...
r tr(nif s_in, unsigned2 c) := transform
  self.pos := c;
	self.s := SALT311.GetNthWord(s_in.instring,c);
	self.xxx_unc := s_in.xxx_unc;
  end;
	
// Split out the words for each wordbag
n := NORMALIZE(	nif, SALT311.WordCount(LEFT.instring), tr(LEFT,COUNTER) );
r2 := RECORD(r)
  INTEGER2 spec;
	END;
...
j1 := IF ( COUNT(keyname)*SIZEOF(pk,max) < SALT311.Config.LookupMaxSize, j1s, j1b_1+j1b_2 ); // Use a pure lookup if you can
...
ifp addw(ifp le, j ri) := TRANSFORM
  SELF.new_instring := TRIM(le.new_instring)+' '+TRIM(ri.s)+' '+(SALT311.StrType)ri.spec;
...
TYPEOF(infile) sb(sf le) := TRANSFORM
  SELF.new_instring := IF(le.instring='','',(SALT311.StrType)le.tot_spec+le.new_instring); // Newstring has space pre-pended by %addw%
  SELF := le;
  END;

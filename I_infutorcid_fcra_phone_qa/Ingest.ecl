...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.phone=RIGHT.phone AND LEFT.persistent_record_id=RIGHT.persistent_record_id AND LEFT.fname=RIGHT.fname AND LEFT.lname=RIGHT.lname AND LEFT.prim_range=RIGHT.prim_range AND LEFT.predir=RIGHT.predir
              AND LEFT.prim_name=RIGHT.prim_name AND LEFT.addr_suffix=RIGHT.addr_suffix AND LEFT.postdir=RIGHT.postdir AND LEFT.unit_desig=RIGHT.unit_desig AND LEFT.sec_range=RIGHT.sec_range AND LEFT.p_city_name=RIGHT.p_city_name AND LEFT.v_city_name=RIGHT.v_city_name AND LEFT.st=RIGHT.st AND LEFT.zip=RIGHT.zip AND LEFT.zip4=RIGHT.zip4
              AND LEFT.rec_type=RIGHT.rec_type AND LEFT.county=RIGHT.county AND LEFT.geo_lat=RIGHT.geo_lat AND LEFT.geo_long=RIGHT.geo_long AND LEFT.geo_blk=RIGHT.geo_blk AND LEFT.did=RIGHT.did AND LEFT.dt_first_seen=RIGHT.dt_first_seen AND LEFT.dt_last_seen=RIGHT.dt_last_seen AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.phone=RIGHT.phone AND LEFT.persistent_record_id=RIGHT.persistent_record_id AND LEFT.fname=RIGHT.fname AND LEFT.lname=RIGHT.lname AND LEFT.prim_range=RIGHT.prim_range AND LEFT.predir=RIGHT.predir
              AND LEFT.prim_name=RIGHT.prim_name AND LEFT.addr_suffix=RIGHT.addr_suffix AND LEFT.postdir=RIGHT.postdir AND LEFT.unit_desig=RIGHT.unit_desig AND LEFT.sec_range=RIGHT.sec_range AND LEFT.p_city_name=RIGHT.p_city_name AND LEFT.v_city_name=RIGHT.v_city_name AND LEFT.st=RIGHT.st AND LEFT.zip=RIGHT.zip AND LEFT.zip4=RIGHT.zip4
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_infutorcid_fcra_phone_qa::Ingest_Cache',EXPIRE(I_infutorcid_fcra_phone_qa.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cluecomm_transaction_log_batch_summary::Ingest_Cache',EXPIRE(I_cluecomm_transaction_log_batch_summary.Config.PersistExpire));

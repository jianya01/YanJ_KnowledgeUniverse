...
LayoutOffender := RECORD
 ...
 blankDataset := DATASET([], LayoutOffender);
 
 fileName := '~thor_data400::key::corrections_offenders_public_qa';
 
EXPORT CorrectionsPublicOffender := IF(COUNT(_Control.LexIDFilterSet) <= 0, 
	INDEX(blankDataset, {sdid}, {blankDataset}, fileName),
	JOIN(DISTRIBUTE(INDEX(blankDataset, {sdid}, {blankDataset}, fileName), HASH64((UNSIGNED8)sdid)), DISTRIBUTE(_Control.LexIDFilterSet(LexID > 0), HASH64(LexID)), (UNSIGNED8)LEFT.sdid = RIGHT.LexID, TRANSFORM(LEFT), LOCAL));
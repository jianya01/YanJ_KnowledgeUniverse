...
fileName := '~thor_data400::key::corrections_punishment_public_qa';
EXPORT KeyCorrectionsPunishment := IF(COUNT(_Control.OffenderKeyFilterSet) <= 0, 
	INDEX(blankDataset, {ok, pt}, {blankDataset}, fileName),
	JOIN(DISTRIBUTE(INDEX(blankDataset, {ok, pt}, {blankDataset}, fileName), HASH64((STRING60)ok)), DISTRIBUTE(_Control.OffenderKeyFilterSet(ofk != ''), HASH64((STRING60)ofk)), LEFT.ok = RIGHT.ofk, TRANSFORM(LEFT), LOCAL));
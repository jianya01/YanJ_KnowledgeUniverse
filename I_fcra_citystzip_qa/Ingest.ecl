...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.zip5=RIGHT.zip5 AND LEFT.zipclass=RIGHT.zipclass AND LEFT.city=RIGHT.city AND LEFT.state=RIGHT.state AND LEFT.county=RIGHT.county AND LEFT.prefctystname=RIGHT.prefctystname
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.zip5=RIGHT.zip5 AND LEFT.zipclass=RIGHT.zipclass AND LEFT.city=RIGHT.city AND LEFT.state=RIGHT.state AND LEFT.county=RIGHT.county AND LEFT.prefctystname=RIGHT.prefctystname
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_fcra_citystzip_qa::Ingest_Cache',EXPIRE(I_fcra_citystzip_qa.Config.PersistExpire));

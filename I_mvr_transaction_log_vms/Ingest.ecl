...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.reference_number=RIGHT.reference_number AND LEFT.date_added=RIGHT.date_added AND LEFT.report_code=RIGHT.report_code AND LEFT.state_postal_code=RIGHT.state_postal_code
              AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.account_id=RIGHT.account_id AND LEFT.customer_number=RIGHT.customer_number AND LEFT.drivers_license_number=RIGHT.drivers_license_number AND LEFT.dfs_number=RIGHT.dfs_number AND LEFT.full_quote_back=RIGHT.full_quote_back AND LEFT.name_prefix=RIGHT.name_prefix AND LEFT.last_name=RIGHT.last_name AND LEFT.first_name=RIGHT.first_name
              AND LEFT.middle_name=RIGHT.middle_name AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.record_version=RIGHT.record_version AND LEFT.dob=RIGHT.dob AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.processing_status=RIGHT.processing_status AND LEFT.report_usage_code=RIGHT.report_usage_code AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.billing_report_code=RIGHT.billing_report_code AND LEFT.price=RIGHT.price
              AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.request_format=RIGHT.request_format AND LEFT.result_format=RIGHT.result_format AND LEFT.result_fullfilling_product=RIGHT.result_fullfilling_product AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_received_date=RIGHT.order_received_date
              AND LEFT.result_returned_date=RIGHT.result_returned_date AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_migrated=RIGHT.is_migrated AND LEFT.is_active=RIGHT.is_active
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.reference_number=RIGHT.reference_number AND LEFT.date_added=RIGHT.date_added AND LEFT.report_code=RIGHT.report_code AND LEFT.state_postal_code=RIGHT.state_postal_code
              AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.account_id=RIGHT.account_id AND LEFT.customer_number=RIGHT.customer_number AND LEFT.drivers_license_number=RIGHT.drivers_license_number AND LEFT.dfs_number=RIGHT.dfs_number AND LEFT.full_quote_back=RIGHT.full_quote_back AND LEFT.name_prefix=RIGHT.name_prefix AND LEFT.last_name=RIGHT.last_name AND LEFT.first_name=RIGHT.first_name
              AND LEFT.middle_name=RIGHT.middle_name AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.record_version=RIGHT.record_version AND LEFT.dob=RIGHT.dob AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.processing_status=RIGHT.processing_status AND LEFT.report_usage_code=RIGHT.report_usage_code AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.billing_report_code=RIGHT.billing_report_code AND LEFT.price=RIGHT.price
              AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.request_format=RIGHT.request_format AND LEFT.result_format=RIGHT.result_format AND LEFT.result_fullfilling_product=RIGHT.result_fullfilling_product AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_received_date=RIGHT.order_received_date
              AND LEFT.result_returned_date=RIGHT.result_returned_date AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_migrated=RIGHT.is_migrated AND LEFT.is_active=RIGHT.is_active
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mvr_transaction_log_vms::Ingest_Cache',EXPIRE(I_mvr_transaction_log_vms.Config.PersistExpire));

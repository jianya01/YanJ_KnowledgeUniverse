...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.service_type=RIGHT.service_type AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.report_code=RIGHT.report_code
              AND LEFT.report_usage=RIGHT.report_usage AND LEFT.requestor=RIGHT.requestor AND LEFT.reference_number=RIGHT.reference_number AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.account_id=RIGHT.account_id AND LEFT.customer_id=RIGHT.customer_id AND LEFT.anchor_transaction_id=RIGHT.anchor_transaction_id AND LEFT.anchor_product_id=RIGHT.anchor_product_id AND LEFT.full_quote_back=RIGHT.full_quote_back
              AND LEFT.i_date_ordered=RIGHT.i_date_ordered AND LEFT.i_addr_house_num=RIGHT.i_addr_house_num AND LEFT.i_addr_apt_num=RIGHT.i_addr_apt_num AND LEFT.i_addr_line=RIGHT.i_addr_line AND LEFT.i_addr_state=RIGHT.i_addr_state AND LEFT.i_addr_zip=RIGHT.i_addr_zip AND LEFT.i_addr_city=RIGHT.i_addr_city AND LEFT.i_addr_county=RIGHT.i_addr_county AND LEFT.i_addr_country=RIGHT.i_addr_country AND LEFT.i_addr_type=RIGHT.i_addr_type
              AND LEFT.addr_household_sur_name=RIGHT.addr_household_sur_name AND LEFT.listed_driver_count=RIGHT.listed_driver_count AND LEFT.confirmed_count=RIGHT.confirmed_count AND LEFT.not_confirmed_count=RIGHT.not_confirmed_count AND LEFT.not_found_count=RIGHT.not_found_count AND LEFT.discovered_surname_count=RIGHT.discovered_surname_count AND LEFT.discovered_surname_exception_count=RIGHT.discovered_surname_exception_count AND LEFT.result_format=RIGHT.result_format AND LEFT.record_version=RIGHT.record_version AND LEFT.processing_status=RIGHT.processing_status
              AND LEFT.inquiry_status=RIGHT.inquiry_status AND LEFT.inquiry_processing_status=RIGHT.inquiry_processing_status AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.price=RIGHT.price AND LEFT.currency=RIGHT.currency AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id
              AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.product_line=RIGHT.product_line AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_active=RIGHT.is_active
              AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.service_type=RIGHT.service_type AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.report_code=RIGHT.report_code
              AND LEFT.report_usage=RIGHT.report_usage AND LEFT.requestor=RIGHT.requestor AND LEFT.reference_number=RIGHT.reference_number AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.account_id=RIGHT.account_id AND LEFT.customer_id=RIGHT.customer_id AND LEFT.anchor_transaction_id=RIGHT.anchor_transaction_id AND LEFT.anchor_product_id=RIGHT.anchor_product_id AND LEFT.full_quote_back=RIGHT.full_quote_back
              AND LEFT.i_date_ordered=RIGHT.i_date_ordered AND LEFT.i_addr_house_num=RIGHT.i_addr_house_num AND LEFT.i_addr_apt_num=RIGHT.i_addr_apt_num AND LEFT.i_addr_line=RIGHT.i_addr_line AND LEFT.i_addr_state=RIGHT.i_addr_state AND LEFT.i_addr_zip=RIGHT.i_addr_zip AND LEFT.i_addr_city=RIGHT.i_addr_city AND LEFT.i_addr_county=RIGHT.i_addr_county AND LEFT.i_addr_country=RIGHT.i_addr_country AND LEFT.i_addr_type=RIGHT.i_addr_type
              AND LEFT.addr_household_sur_name=RIGHT.addr_household_sur_name AND LEFT.listed_driver_count=RIGHT.listed_driver_count AND LEFT.confirmed_count=RIGHT.confirmed_count AND LEFT.not_confirmed_count=RIGHT.not_confirmed_count AND LEFT.not_found_count=RIGHT.not_found_count AND LEFT.discovered_surname_count=RIGHT.discovered_surname_count AND LEFT.discovered_surname_exception_count=RIGHT.discovered_surname_exception_count AND LEFT.result_format=RIGHT.result_format AND LEFT.record_version=RIGHT.record_version AND LEFT.processing_status=RIGHT.processing_status
              AND LEFT.inquiry_status=RIGHT.inquiry_status AND LEFT.inquiry_processing_status=RIGHT.inquiry_processing_status AND LEFT.billing_type_id=RIGHT.billing_type_id AND LEFT.price=RIGHT.price AND LEFT.currency=RIGHT.currency AND LEFT.pricing_error_code=RIGHT.pricing_error_code AND LEFT.free=RIGHT.free AND LEFT.report_options=RIGHT.report_options AND LEFT.transaction_code=RIGHT.transaction_code AND LEFT.return_node_id=RIGHT.return_node_id
              AND LEFT.request_node_id=RIGHT.request_node_id AND LEFT.order_status_code=RIGHT.order_status_code AND LEFT.product_line=RIGHT.product_line AND LEFT.login_history_id=RIGHT.login_history_id AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.is_active=RIGHT.is_active
              AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_dd_transaction_log::Ingest_Cache',EXPIRE(I_dd_transaction_log.Config.PersistExpire));

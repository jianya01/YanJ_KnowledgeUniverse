...
fileName := '~vault::thor::key::prod::relatives_v3_qa';
EXPORT key_relatives_v3 := IF(COUNT(_Control.LexIDFilterSet) <= 0, 
	DATASET(fileName, fileLayout, THOR, __COMPRESSED__),
	JOIN(DISTRIBUTE(DATASET(fileName, fileLayout, THOR, __COMPRESSED__), HASH64((UNSIGNED8)DID1)), DISTRIBUTE(_Control.LexIDFilterSet(LexID > 0), HASH64(LexID)), (UNSIGNED8)LEFT.DID1 = RIGHT.LexID, TRANSFORM(LEFT), LOCAL));
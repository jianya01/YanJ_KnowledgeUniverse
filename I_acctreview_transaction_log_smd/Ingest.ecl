...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.reject_code=RIGHT.reject_code AND LEFT.model=RIGHT.model AND LEFT.version=RIGHT.version AND LEFT.reason_code=RIGHT.reason_code
              AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_added=RIGHT.date_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.score=RIGHT.score AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.reject_code=RIGHT.reject_code AND LEFT.model=RIGHT.model AND LEFT.version=RIGHT.version AND LEFT.reason_code=RIGHT.reason_code
              AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_added=RIGHT.date_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.score=RIGHT.score AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_acctreview_transaction_log_smd::Ingest_Cache',EXPIRE(I_acctreview_transaction_log_smd.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.ssn=RIGHT.ssn AND LEFT.did=RIGHT.did AND LEFT.did_score=RIGHT.did_score AND LEFT.filedate=RIGHT.filedate AND LEFT.rec_type=RIGHT.rec_type AND LEFT.rec_type_orig=RIGHT.rec_type_orig
              AND LEFT.lname=RIGHT.lname AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.fname=RIGHT.fname AND LEFT.mname=RIGHT.mname AND LEFT.vorp_code=RIGHT.vorp_code AND LEFT.dod8=RIGHT.dod8 AND LEFT.dob8=RIGHT.dob8 AND LEFT.st_country_code=RIGHT.st_country_code AND LEFT.zip_lastres=RIGHT.zip_lastres AND LEFT.zip_lastpayment=RIGHT.zip_lastpayment
              AND LEFT.state=RIGHT.state AND LEFT.fipscounty=RIGHT.fipscounty AND LEFT.crlf=RIGHT.crlf AND LEFT.state_death_flag=RIGHT.state_death_flag AND LEFT.death_rec_src=RIGHT.death_rec_src AND LEFT.state_death_id=RIGHT.state_death_id AND LEFT.src=RIGHT.src AND LEFT.glb_flag=RIGHT.glb_flag AND LEFT.__internal_fpos__=RIGHT.__internal_fpos__ AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.ssn=RIGHT.ssn AND LEFT.did=RIGHT.did AND LEFT.did_score=RIGHT.did_score AND LEFT.filedate=RIGHT.filedate AND LEFT.rec_type=RIGHT.rec_type AND LEFT.rec_type_orig=RIGHT.rec_type_orig
              AND LEFT.lname=RIGHT.lname AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.fname=RIGHT.fname AND LEFT.mname=RIGHT.mname AND LEFT.vorp_code=RIGHT.vorp_code AND LEFT.dod8=RIGHT.dod8 AND LEFT.dob8=RIGHT.dob8 AND LEFT.st_country_code=RIGHT.st_country_code AND LEFT.zip_lastres=RIGHT.zip_lastres AND LEFT.zip_lastpayment=RIGHT.zip_lastpayment
              AND LEFT.state=RIGHT.state AND LEFT.fipscounty=RIGHT.fipscounty AND LEFT.crlf=RIGHT.crlf AND LEFT.state_death_flag=RIGHT.state_death_flag AND LEFT.death_rec_src=RIGHT.death_rec_src AND LEFT.state_death_id=RIGHT.state_death_id AND LEFT.src=RIGHT.src AND LEFT.glb_flag=RIGHT.glb_flag AND LEFT.__internal_fpos__=RIGHT.__internal_fpos__ AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_fcra_death_master_ssa_ssn::Ingest_Cache',EXPIRE(I_fcra_death_master_ssa_ssn.Config.PersistExpire));

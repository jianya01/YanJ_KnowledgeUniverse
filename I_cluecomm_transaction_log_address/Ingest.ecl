...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.address_type=RIGHT.address_type AND LEFT.house_number=RIGHT.house_number AND LEFT.street_name=RIGHT.street_name AND LEFT.apt_no=RIGHT.apt_no
              AND LEFT.city=RIGHT.city AND LEFT.state=RIGHT.state AND LEFT.zip=RIGHT.zip AND LEFT.zip_ext=RIGHT.zip_ext AND LEFT.date_added=RIGHT.date_added AND LEFT.potentialcorruption=RIGHT.potentialcorruption AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.address_type=RIGHT.address_type AND LEFT.house_number=RIGHT.house_number AND LEFT.street_name=RIGHT.street_name AND LEFT.apt_no=RIGHT.apt_no
              AND LEFT.city=RIGHT.city AND LEFT.state=RIGHT.state AND LEFT.zip=RIGHT.zip AND LEFT.zip_ext=RIGHT.zip_ext AND LEFT.date_added=RIGHT.date_added AND LEFT.potentialcorruption=RIGHT.potentialcorruption AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cluecomm_transaction_log_address::Ingest_Cache',EXPIRE(I_cluecomm_transaction_log_address.Config.PersistExpire));

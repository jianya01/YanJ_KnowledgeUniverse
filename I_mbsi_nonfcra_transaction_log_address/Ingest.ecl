...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.person_seq=RIGHT.person_seq AND LEFT.business_seq=RIGHT.business_seq AND LEFT.address_seq=RIGHT.address_seq AND LEFT.address_classification=RIGHT.address_classification
              AND LEFT.address_type=RIGHT.address_type AND LEFT.address_line1=RIGHT.address_line1 AND LEFT.address_line2=RIGHT.address_line2 AND LEFT.address_line3=RIGHT.address_line3 AND LEFT.address_house_no=RIGHT.address_house_no AND LEFT.address_street_name=RIGHT.address_street_name AND LEFT.address_apt_no=RIGHT.address_apt_no AND LEFT.address_city=RIGHT.address_city AND LEFT.address_state=RIGHT.address_state AND LEFT.address_zip=RIGHT.address_zip
              AND LEFT.address_zip_ext=RIGHT.address_zip_ext AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.person_seq=RIGHT.person_seq AND LEFT.business_seq=RIGHT.business_seq AND LEFT.address_seq=RIGHT.address_seq AND LEFT.address_classification=RIGHT.address_classification
              AND LEFT.address_type=RIGHT.address_type AND LEFT.address_line1=RIGHT.address_line1 AND LEFT.address_line2=RIGHT.address_line2 AND LEFT.address_line3=RIGHT.address_line3 AND LEFT.address_house_no=RIGHT.address_house_no AND LEFT.address_street_name=RIGHT.address_street_name AND LEFT.address_apt_no=RIGHT.address_apt_no AND LEFT.address_city=RIGHT.address_city AND LEFT.address_state=RIGHT.address_state AND LEFT.address_zip=RIGHT.address_zip
              AND LEFT.address_zip_ext=RIGHT.address_zip_ext AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_address::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_address.Config.PersistExpire));

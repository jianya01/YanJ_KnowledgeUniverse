...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.status=RIGHT.status AND LEFT.state=RIGHT.state AND LEFT.am_best=RIGHT.am_best AND LEFT.policy_number=RIGHT.policy_number
              AND LEFT.policy_type=RIGHT.policy_type AND LEFT.policy_inception_date=RIGHT.policy_inception_date AND LEFT.policy_begin_date=RIGHT.policy_begin_date AND LEFT.policy_cancel_date=RIGHT.policy_cancel_date AND LEFT.policy_end_date=RIGHT.policy_end_date AND LEFT.last_cancel_reason=RIGHT.last_cancel_reason AND LEFT.insurance_type=RIGHT.insurance_type AND LEFT.suppressed=RIGHT.suppressed AND LEFT.naic=RIGHT.naic AND LEFT.risk_type=RIGHT.risk_type
              AND LEFT.carrier_name=RIGHT.carrier_name AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.status=RIGHT.status AND LEFT.state=RIGHT.state AND LEFT.am_best=RIGHT.am_best AND LEFT.policy_number=RIGHT.policy_number
              AND LEFT.policy_type=RIGHT.policy_type AND LEFT.policy_inception_date=RIGHT.policy_inception_date AND LEFT.policy_begin_date=RIGHT.policy_begin_date AND LEFT.policy_cancel_date=RIGHT.policy_cancel_date AND LEFT.policy_end_date=RIGHT.policy_end_date AND LEFT.last_cancel_reason=RIGHT.last_cancel_reason AND LEFT.insurance_type=RIGHT.insurance_type AND LEFT.suppressed=RIGHT.suppressed AND LEFT.naic=RIGHT.naic AND LEFT.risk_type=RIGHT.risk_type
              AND LEFT.carrier_name=RIGHT.carrier_name AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cc_transaction_log_policy::Ingest_Cache',EXPIRE(I_cc_transaction_log_policy.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.reference_number=RIGHT.reference_number AND LEFT.status=RIGHT.status AND LEFT.reorder_status=RIGHT.reorder_status AND LEFT.review_date=RIGHT.review_date
              AND LEFT.i_person_first_name=RIGHT.i_person_first_name AND LEFT.i_person_last_name=RIGHT.i_person_last_name AND LEFT.i_person_middle_name=RIGHT.i_person_middle_name AND LEFT.i_person_name_suffix=RIGHT.i_person_name_suffix AND LEFT.i_addr_line=RIGHT.i_addr_line AND LEFT.i_addr_city=RIGHT.i_addr_city AND LEFT.i_addr_state=RIGHT.i_addr_state AND LEFT.i_addr_zip=RIGHT.i_addr_zip AND LEFT.i_addr_zip_ext=RIGHT.i_addr_zip_ext AND LEFT.i_person_ssn=RIGHT.i_person_ssn
              AND LEFT.i_person_dob=RIGHT.i_person_dob AND LEFT.customer_number=RIGHT.customer_number AND LEFT.vendor=RIGHT.vendor AND LEFT.member_number=RIGHT.member_number AND LEFT.i_spouse_first_name=RIGHT.i_spouse_first_name AND LEFT.i_spouse_last_name=RIGHT.i_spouse_last_name AND LEFT.i_spouse_middle_name=RIGHT.i_spouse_middle_name AND LEFT.i_spouse_name_suffix=RIGHT.i_spouse_name_suffix AND LEFT.login_id=RIGHT.login_id AND LEFT.is_active=RIGHT.is_active
              AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_added=RIGHT.date_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.reference_number=RIGHT.reference_number AND LEFT.status=RIGHT.status AND LEFT.reorder_status=RIGHT.reorder_status AND LEFT.review_date=RIGHT.review_date
              AND LEFT.i_person_first_name=RIGHT.i_person_first_name AND LEFT.i_person_last_name=RIGHT.i_person_last_name AND LEFT.i_person_middle_name=RIGHT.i_person_middle_name AND LEFT.i_person_name_suffix=RIGHT.i_person_name_suffix AND LEFT.i_addr_line=RIGHT.i_addr_line AND LEFT.i_addr_city=RIGHT.i_addr_city AND LEFT.i_addr_state=RIGHT.i_addr_state AND LEFT.i_addr_zip=RIGHT.i_addr_zip AND LEFT.i_addr_zip_ext=RIGHT.i_addr_zip_ext AND LEFT.i_person_ssn=RIGHT.i_person_ssn
              AND LEFT.i_person_dob=RIGHT.i_person_dob AND LEFT.customer_number=RIGHT.customer_number AND LEFT.vendor=RIGHT.vendor AND LEFT.member_number=RIGHT.member_number AND LEFT.i_spouse_first_name=RIGHT.i_spouse_first_name AND LEFT.i_spouse_last_name=RIGHT.i_spouse_last_name AND LEFT.i_spouse_middle_name=RIGHT.i_spouse_middle_name AND LEFT.i_spouse_name_suffix=RIGHT.i_spouse_name_suffix AND LEFT.login_id=RIGHT.login_id AND LEFT.is_active=RIGHT.is_active
              AND LEFT.user_added=RIGHT.user_added AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_added=RIGHT.date_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_acctreview_transaction_log::Ingest_Cache',EXPIRE(I_acctreview_transaction_log.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.report_code=RIGHT.report_code AND LEFT.account_base=RIGHT.account_base AND LEFT.account_suffix=RIGHT.account_suffix AND LEFT.customer_number=RIGHT.customer_number
              AND LEFT.am_best_number=RIGHT.am_best_number AND LEFT.anchor_product=RIGHT.anchor_product AND LEFT.full_quote_back=RIGHT.full_quote_back AND LEFT.name_prefix=RIGHT.name_prefix AND LEFT.last_name=RIGHT.last_name AND LEFT.first_name=RIGHT.first_name AND LEFT.middle_name=RIGHT.middle_name AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.ssn=RIGHT.ssn AND LEFT.count_subject_in=RIGHT.count_subject_in
              AND LEFT.dl_current=RIGHT.dl_current AND LEFT.dl_current_state=RIGHT.dl_current_state AND LEFT.dl_prior=RIGHT.dl_prior AND LEFT.dl_prior_state=RIGHT.dl_prior_state AND LEFT.state_subject=RIGHT.state_subject AND LEFT.record_version=RIGHT.record_version AND LEFT.dob=RIGHT.dob AND LEFT.special_billing_id=RIGHT.special_billing_id AND LEFT.date_ordered=RIGHT.date_ordered AND LEFT.reference_number=RIGHT.reference_number
              ...
              AND LEFT.vehicle_state=RIGHT.vehicle_state AND LEFT.risk_full_address=RIGHT.risk_full_address AND LEFT.current_full_address=RIGHT.current_full_address AND LEFT.mail_full_address=RIGHT.mail_full_address AND LEFT.prior_full_address=RIGHT.prior_full_address AND LEFT.addresses_used=RIGHT.addresses_used AND LEFT.policies_used=RIGHT.policies_used AND LEFT.policies_returned=RIGHT.policies_returned AND LEFT.policy_number=RIGHT.policy_number AND LEFT.login_history_id=RIGHT.login_history_id
              AND LEFT.ip_address=RIGHT.ip_address AND LEFT.response_time=RIGHT.response_time AND LEFT.esp_method=RIGHT.esp_method AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.user_added=RIGHT.user_added AND LEFT.date_added=RIGHT.date_added AND LEFT.is_active=RIGHT.is_active AND LEFT.user_changed=RIGHT.user_changed AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_CCFC_transaction_log::Ingest_Cache',EXPIRE(I_CCFC_transaction_log.Config.PersistExpire));

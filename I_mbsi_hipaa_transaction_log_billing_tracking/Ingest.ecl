...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.request_id=RIGHT.request_id AND LEFT.search_transaction_id=RIGHT.search_transaction_id AND LEFT.search_date_added=RIGHT.search_date_added AND LEFT.search_expiration_date=RIGHT.search_expiration_date AND LEFT.report_billable_transaction_id=RIGHT.report_billable_transaction_id AND LEFT.report_billable_date_added=RIGHT.report_billable_date_added
              AND LEFT.date_added=RIGHT.date_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.request_id=RIGHT.request_id AND LEFT.search_transaction_id=RIGHT.search_transaction_id AND LEFT.search_date_added=RIGHT.search_date_added AND LEFT.search_expiration_date=RIGHT.search_expiration_date AND LEFT.report_billable_transaction_id=RIGHT.report_billable_transaction_id AND LEFT.report_billable_date_added=RIGHT.report_billable_date_added
              AND LEFT.date_added=RIGHT.date_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_hipaa_transaction_log_billing_tracking::Ingest_Cache',EXPIRE(I_mbsi_hipaa_transaction_log_billing_tracking.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.policy_sequence=RIGHT.policy_sequence AND LEFT.sequence=RIGHT.sequence AND LEFT.unit_number=RIGHT.unit_number AND LEFT.last_name=RIGHT.last_name AND LEFT.first_name=RIGHT.first_name
              AND LEFT.relationship=RIGHT.relationship AND LEFT.subject_level=RIGHT.subject_level AND LEFT.subject_begin_date=RIGHT.subject_begin_date AND LEFT.subject_end_date=RIGHT.subject_end_date AND LEFT.start_date=RIGHT.start_date AND LEFT.ocurrences=RIGHT.ocurrences AND LEFT.last_cancel_date=RIGHT.last_cancel_date AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.policy_sequence=RIGHT.policy_sequence AND LEFT.sequence=RIGHT.sequence AND LEFT.unit_number=RIGHT.unit_number AND LEFT.last_name=RIGHT.last_name AND LEFT.first_name=RIGHT.first_name
              AND LEFT.relationship=RIGHT.relationship AND LEFT.subject_level=RIGHT.subject_level AND LEFT.subject_begin_date=RIGHT.subject_begin_date AND LEFT.subject_end_date=RIGHT.subject_end_date AND LEFT.start_date=RIGHT.start_date AND LEFT.ocurrences=RIGHT.ocurrences AND LEFT.last_cancel_date=RIGHT.last_cancel_date AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cc_transaction_log_policy_subject::Ingest_Cache',EXPIRE(I_cc_transaction_log_policy_subject.Config.PersistExpire));

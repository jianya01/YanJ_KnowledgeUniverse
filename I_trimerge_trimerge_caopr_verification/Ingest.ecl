...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.verification_id=RIGHT.verification_id AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.credit_public_record_id=RIGHT.credit_public_record_id AND LEFT.caopr_record_type_id=RIGHT.caopr_record_type_id AND LEFT.record_ordered=RIGHT.record_ordered AND LEFT.search_results=RIGHT.search_results
              AND LEFT.type_of_search=RIGHT.type_of_search AND LEFT.date_of_search=RIGHT.date_of_search AND LEFT.search_period_start=RIGHT.search_period_start AND LEFT.search_period_end=RIGHT.search_period_end AND LEFT.address_covered=RIGHT.address_covered AND LEFT.court_name=RIGHT.court_name AND LEFT.court_location=RIGHT.court_location AND LEFT.subject_first_name=RIGHT.subject_first_name AND LEFT.subject_middle_name=RIGHT.subject_middle_name AND LEFT.subject_last_name=RIGHT.subject_last_name
              AND LEFT.subject_suffix=RIGHT.subject_suffix AND LEFT.subject_ssn=RIGHT.subject_ssn AND LEFT.subject_address=RIGHT.subject_address AND LEFT.subject_verified=RIGHT.subject_verified AND LEFT.comments=RIGHT.comments AND LEFT.remarks=RIGHT.remarks AND LEFT.is_verified=RIGHT.is_verified AND LEFT.date_verified=RIGHT.date_verified AND LEFT.user_verified=RIGHT.user_verified AND LEFT.date_added=RIGHT.date_added
              AND LEFT.user_added=RIGHT.user_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.user_changed=RIGHT.user_changed AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.verification_id=RIGHT.verification_id AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.credit_public_record_id=RIGHT.credit_public_record_id AND LEFT.caopr_record_type_id=RIGHT.caopr_record_type_id AND LEFT.record_ordered=RIGHT.record_ordered AND LEFT.search_results=RIGHT.search_results
              AND LEFT.type_of_search=RIGHT.type_of_search AND LEFT.date_of_search=RIGHT.date_of_search AND LEFT.search_period_start=RIGHT.search_period_start AND LEFT.search_period_end=RIGHT.search_period_end AND LEFT.address_covered=RIGHT.address_covered AND LEFT.court_name=RIGHT.court_name AND LEFT.court_location=RIGHT.court_location AND LEFT.subject_first_name=RIGHT.subject_first_name AND LEFT.subject_middle_name=RIGHT.subject_middle_name AND LEFT.subject_last_name=RIGHT.subject_last_name
              AND LEFT.subject_suffix=RIGHT.subject_suffix AND LEFT.subject_ssn=RIGHT.subject_ssn AND LEFT.subject_address=RIGHT.subject_address AND LEFT.subject_verified=RIGHT.subject_verified AND LEFT.comments=RIGHT.comments AND LEFT.remarks=RIGHT.remarks AND LEFT.is_verified=RIGHT.is_verified AND LEFT.date_verified=RIGHT.date_verified AND LEFT.user_verified=RIGHT.user_verified AND LEFT.date_added=RIGHT.date_added
              AND LEFT.user_added=RIGHT.user_added AND LEFT.date_changed=RIGHT.date_changed AND LEFT.user_changed=RIGHT.user_changed AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_trimerge_trimerge_caopr_verification::Ingest_Cache',EXPIRE(I_trimerge_trimerge_caopr_verification.Config.PersistExpire));

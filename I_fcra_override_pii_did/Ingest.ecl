...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.s_did=RIGHT.s_did AND LEFT.uid=RIGHT.uid AND LEFT.date_created=RIGHT.date_created AND LEFT.dt_first_seen=RIGHT.dt_first_seen AND LEFT.dt_last_seen=RIGHT.dt_last_seen AND LEFT.did=RIGHT.did
              AND LEFT.fname=RIGHT.fname AND LEFT.mname=RIGHT.mname AND LEFT.lname=RIGHT.lname AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.ssn=RIGHT.ssn AND LEFT.dob=RIGHT.dob AND LEFT.predir=RIGHT.predir AND LEFT.prim_name=RIGHT.prim_name AND LEFT.prim_range=RIGHT.prim_range AND LEFT.suffix=RIGHT.suffix
              AND LEFT.postdir=RIGHT.postdir AND LEFT.unit_desig=RIGHT.unit_desig AND LEFT.sec_range=RIGHT.sec_range AND LEFT.zip4=RIGHT.zip4 AND LEFT.address=RIGHT.address AND LEFT.city_name=RIGHT.city_name AND LEFT.st=RIGHT.st AND LEFT.zip=RIGHT.zip AND LEFT.phone=RIGHT.phone AND LEFT.dl_number=RIGHT.dl_number
              AND LEFT.dl_state=RIGHT.dl_state AND LEFT.__internal_fpos__=RIGHT.__internal_fpos__ AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.s_did=RIGHT.s_did AND LEFT.uid=RIGHT.uid AND LEFT.date_created=RIGHT.date_created AND LEFT.dt_first_seen=RIGHT.dt_first_seen AND LEFT.dt_last_seen=RIGHT.dt_last_seen AND LEFT.did=RIGHT.did
              AND LEFT.fname=RIGHT.fname AND LEFT.mname=RIGHT.mname AND LEFT.lname=RIGHT.lname AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.ssn=RIGHT.ssn AND LEFT.dob=RIGHT.dob AND LEFT.predir=RIGHT.predir AND LEFT.prim_name=RIGHT.prim_name AND LEFT.prim_range=RIGHT.prim_range AND LEFT.suffix=RIGHT.suffix
              AND LEFT.postdir=RIGHT.postdir AND LEFT.unit_desig=RIGHT.unit_desig AND LEFT.sec_range=RIGHT.sec_range AND LEFT.zip4=RIGHT.zip4 AND LEFT.address=RIGHT.address AND LEFT.city_name=RIGHT.city_name AND LEFT.st=RIGHT.st AND LEFT.zip=RIGHT.zip AND LEFT.phone=RIGHT.phone AND LEFT.dl_number=RIGHT.dl_number
              AND LEFT.dl_state=RIGHT.dl_state AND LEFT.__internal_fpos__=RIGHT.__internal_fpos__ AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_fcra_override_pii_did::Ingest_Cache',EXPIRE(I_fcra_override_pii_did.Config.PersistExpire));

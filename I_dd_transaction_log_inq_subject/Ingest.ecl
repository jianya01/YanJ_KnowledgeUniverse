...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.inq_last_name=RIGHT.inq_last_name AND LEFT.inq_first_name=RIGHT.inq_first_name AND LEFT.inq_middle_name=RIGHT.inq_middle_name AND LEFT.inq_suffix=RIGHT.inq_suffix
              AND LEFT.inq_license_num=RIGHT.inq_license_num AND LEFT.inq_license_state=RIGHT.inq_license_state AND LEFT.inq_dob=RIGHT.inq_dob AND LEFT.inq_gender=RIGHT.inq_gender AND LEFT.inq_ssn=RIGHT.inq_ssn AND LEFT.inq_idl=RIGHT.inq_idl AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.date_added=RIGHT.date_added AND LEFT.inq_last_name=RIGHT.inq_last_name AND LEFT.inq_first_name=RIGHT.inq_first_name AND LEFT.inq_middle_name=RIGHT.inq_middle_name AND LEFT.inq_suffix=RIGHT.inq_suffix
              AND LEFT.inq_license_num=RIGHT.inq_license_num AND LEFT.inq_license_state=RIGHT.inq_license_state AND LEFT.inq_dob=RIGHT.inq_dob AND LEFT.inq_gender=RIGHT.inq_gender AND LEFT.inq_ssn=RIGHT.inq_ssn AND LEFT.inq_idl=RIGHT.inq_idl AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_dd_transaction_log_inq_subject::Ingest_Cache',EXPIRE(I_dd_transaction_log_inq_subject.Config.PersistExpire));

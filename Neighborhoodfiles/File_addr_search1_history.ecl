...
layout_addr_search1_history := RECORD
  string5 zip;
  string10 prim_range;
  string28 prim_name;
  string4 addr_suffix;
...
Addr_search1 := DATASET([], layout_addr_search1_history); // Currently we don't have the base file in the Vault, only the KEY file.  As such we can't rebuild this index in the Vault - setting the base dataset to blank

EXPORT File_addr_search1_history := INDEX(Addr_search1, 
							 {zip,prim_range,prim_name,addr_suffix,predir,postdir,sec_range},
							 {Addr_search1}, '~thor_data400::key::advo::qa::addr_search1_history');
							 
						//	output(choosen(File_addr_search1_history,10));
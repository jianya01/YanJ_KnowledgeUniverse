...
layout_fcra_addr_search1_history  :=  RECORD
  string5 zip;
  string10 prim_range;
  string28 prim_name;
  string4 addr_suffix;
...
Advo_fcra_addr_search1_history := DATASET([], layout_fcra_addr_search1_history); // Currently we don't have the base file in the Vault, only the KEY file.  As such we can't rebuild this index in the Vault - setting the base dataset to blank

 EXPORT  file_advo_fcra_addr_search1_history := INDEX(Advo_fcra_addr_search1_history, 
							  {zip,prim_range,prim_name,addr_suffix,predir,postdir,sec_range},
							 {Advo_fcra_addr_search1_history}, '~thor_data400::key::advo::fcra::qa::addr_search1_history');
							 							 			
										//	output(choosen(file_advo_fcra_addr_search1_history,10));	

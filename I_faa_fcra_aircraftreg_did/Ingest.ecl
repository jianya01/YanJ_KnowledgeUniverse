...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.did=RIGHT.did AND LEFT.n_number=RIGHT.n_number AND LEFT.aircraft_id=RIGHT.aircraft_id AND LEFT.persistent_record_id=RIGHT.persistent_record_id AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_faa_fcra_aircraftreg_did::Ingest_Cache',EXPIRE(I_faa_fcra_aircraftreg_did.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence_smd=RIGHT.sequence_smd AND LEFT.person_type=RIGHT.person_type AND LEFT.extension_type=RIGHT.extension_type AND LEFT.sequence=RIGHT.sequence AND LEFT.value=RIGHT.value
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence_smd=RIGHT.sequence_smd AND LEFT.person_type=RIGHT.person_type AND LEFT.extension_type=RIGHT.extension_type AND LEFT.sequence=RIGHT.sequence AND LEFT.value=RIGHT.value
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_trimerge_transaction_log_extension::Ingest_Cache',EXPIRE(I_trimerge_transaction_log_extension.Config.PersistExpire));

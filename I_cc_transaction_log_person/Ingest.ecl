...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.first_name=RIGHT.first_name AND LEFT.middle_name=RIGHT.middle_name AND LEFT.last_name=RIGHT.last_name AND LEFT.ssn=RIGHT.ssn
              AND LEFT.current_dl_state=RIGHT.current_dl_state AND LEFT.current_dl_number=RIGHT.current_dl_number AND LEFT.prior_dl_state=RIGHT.prior_dl_state AND LEFT.prior_dl_number=RIGHT.prior_dl_number AND LEFT.dob=RIGHT.dob AND LEFT.addr_1_class_code=RIGHT.addr_1_class_code AND LEFT.addr_1_house_number=RIGHT.addr_1_house_number AND LEFT.addr_1_street_name=RIGHT.addr_1_street_name AND LEFT.addr_1_city=RIGHT.addr_1_city AND LEFT.addr_1_state=RIGHT.addr_1_state
              AND LEFT.addr_1_zip=RIGHT.addr_1_zip AND LEFT.addr_1_zip4=RIGHT.addr_1_zip4 AND LEFT.addr_1_apt_num=RIGHT.addr_1_apt_num AND LEFT.addr_2_class_code=RIGHT.addr_2_class_code AND LEFT.addr_2_house_number=RIGHT.addr_2_house_number AND LEFT.addr_2_street_name=RIGHT.addr_2_street_name AND LEFT.addr_2_city=RIGHT.addr_2_city AND LEFT.addr_2_state=RIGHT.addr_2_state AND LEFT.addr_2_zip=RIGHT.addr_2_zip AND LEFT.addr_2_zip4=RIGHT.addr_2_zip4
              AND LEFT.addr_2_apt_num=RIGHT.addr_2_apt_num AND LEFT.addr_3_class_code=RIGHT.addr_3_class_code AND LEFT.addr_3_house_number=RIGHT.addr_3_house_number AND LEFT.addr_3_street_name=RIGHT.addr_3_street_name AND LEFT.addr_3_city=RIGHT.addr_3_city AND LEFT.addr_3_state=RIGHT.addr_3_state AND LEFT.addr_3_zip=RIGHT.addr_3_zip AND LEFT.addr_3_zip4=RIGHT.addr_3_zip4 AND LEFT.addr_3_apt_num=RIGHT.addr_3_apt_num AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.first_name=RIGHT.first_name AND LEFT.middle_name=RIGHT.middle_name AND LEFT.last_name=RIGHT.last_name AND LEFT.ssn=RIGHT.ssn
              AND LEFT.current_dl_state=RIGHT.current_dl_state AND LEFT.current_dl_number=RIGHT.current_dl_number AND LEFT.prior_dl_state=RIGHT.prior_dl_state AND LEFT.prior_dl_number=RIGHT.prior_dl_number AND LEFT.dob=RIGHT.dob AND LEFT.addr_1_class_code=RIGHT.addr_1_class_code AND LEFT.addr_1_house_number=RIGHT.addr_1_house_number AND LEFT.addr_1_street_name=RIGHT.addr_1_street_name AND LEFT.addr_1_city=RIGHT.addr_1_city AND LEFT.addr_1_state=RIGHT.addr_1_state
              AND LEFT.addr_1_zip=RIGHT.addr_1_zip AND LEFT.addr_1_zip4=RIGHT.addr_1_zip4 AND LEFT.addr_1_apt_num=RIGHT.addr_1_apt_num AND LEFT.addr_2_class_code=RIGHT.addr_2_class_code AND LEFT.addr_2_house_number=RIGHT.addr_2_house_number AND LEFT.addr_2_street_name=RIGHT.addr_2_street_name AND LEFT.addr_2_city=RIGHT.addr_2_city AND LEFT.addr_2_state=RIGHT.addr_2_state AND LEFT.addr_2_zip=RIGHT.addr_2_zip AND LEFT.addr_2_zip4=RIGHT.addr_2_zip4
              AND LEFT.addr_2_apt_num=RIGHT.addr_2_apt_num AND LEFT.addr_3_class_code=RIGHT.addr_3_class_code AND LEFT.addr_3_house_number=RIGHT.addr_3_house_number AND LEFT.addr_3_street_name=RIGHT.addr_3_street_name AND LEFT.addr_3_city=RIGHT.addr_3_city AND LEFT.addr_3_state=RIGHT.addr_3_state AND LEFT.addr_3_zip=RIGHT.addr_3_zip AND LEFT.addr_3_zip4=RIGHT.addr_3_zip4 AND LEFT.addr_3_apt_num=RIGHT.addr_3_apt_num AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_cc_transaction_log_person::Ingest_Cache',EXPIRE(I_cc_transaction_log_person.Config.PersistExpire));

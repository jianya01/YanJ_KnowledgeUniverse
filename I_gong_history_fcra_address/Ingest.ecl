...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.prim_name=RIGHT.prim_name AND LEFT.st=RIGHT.st AND LEFT.z5=RIGHT.z5 AND LEFT.prim_range=RIGHT.prim_range AND LEFT.sec_range=RIGHT.sec_range AND LEFT.current_flag=RIGHT.current_flag
              AND LEFT.business_flag=RIGHT.business_flag AND LEFT.src=RIGHT.src AND LEFT.listing_type_bus=RIGHT.listing_type_bus AND LEFT.listing_type_res=RIGHT.listing_type_res AND LEFT.listing_type_gov=RIGHT.listing_type_gov AND LEFT.publish_code=RIGHT.publish_code AND LEFT.prior_area_code=RIGHT.prior_area_code AND LEFT.phone10=RIGHT.phone10 AND LEFT.predir=RIGHT.predir AND LEFT.suffix=RIGHT.suffix
              AND LEFT.postdir=RIGHT.postdir AND LEFT.unit_desig=RIGHT.unit_desig AND LEFT.p_city_name=RIGHT.p_city_name AND LEFT.v_predir=RIGHT.v_predir AND LEFT.v_prim_name=RIGHT.v_prim_name AND LEFT.v_suffix=RIGHT.v_suffix AND LEFT.v_postdir=RIGHT.v_postdir AND LEFT.v_city_name=RIGHT.v_city_name AND LEFT.z4=RIGHT.z4 AND LEFT.county_code=RIGHT.county_code
              AND LEFT.geo_lat=RIGHT.geo_lat AND LEFT.geo_long=RIGHT.geo_long AND LEFT.msa=RIGHT.msa AND LEFT.designation=RIGHT.designation AND LEFT.name_prefix=RIGHT.name_prefix AND LEFT.name_first=RIGHT.name_first AND LEFT.name_middle=RIGHT.name_middle AND LEFT.name_last=RIGHT.name_last AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.listed_name=RIGHT.listed_name
              AND LEFT.caption_text=RIGHT.caption_text AND LEFT.omit_address=RIGHT.omit_address AND LEFT.omit_phone=RIGHT.omit_phone AND LEFT.omit_locality=RIGHT.omit_locality AND LEFT.see_also_text=RIGHT.see_also_text AND LEFT.did=RIGHT.did AND LEFT.hhid=RIGHT.hhid AND LEFT.bdid=RIGHT.bdid AND LEFT.dt_first_seen=RIGHT.dt_first_seen AND LEFT.dt_last_seen=RIGHT.dt_last_seen
              AND LEFT.current_record_flag=RIGHT.current_record_flag AND LEFT.deletion_date=RIGHT.deletion_date AND LEFT.disc_cnt6=RIGHT.disc_cnt6 AND LEFT.disc_cnt12=RIGHT.disc_cnt12 AND LEFT.disc_cnt18=RIGHT.disc_cnt18 AND LEFT.persistent_record_id=RIGHT.persistent_record_id AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.prim_name=RIGHT.prim_name AND LEFT.st=RIGHT.st AND LEFT.z5=RIGHT.z5 AND LEFT.prim_range=RIGHT.prim_range AND LEFT.sec_range=RIGHT.sec_range AND LEFT.current_flag=RIGHT.current_flag
              AND LEFT.business_flag=RIGHT.business_flag AND LEFT.src=RIGHT.src AND LEFT.listing_type_bus=RIGHT.listing_type_bus AND LEFT.listing_type_res=RIGHT.listing_type_res AND LEFT.listing_type_gov=RIGHT.listing_type_gov AND LEFT.publish_code=RIGHT.publish_code AND LEFT.prior_area_code=RIGHT.prior_area_code AND LEFT.phone10=RIGHT.phone10 AND LEFT.predir=RIGHT.predir AND LEFT.suffix=RIGHT.suffix
              AND LEFT.postdir=RIGHT.postdir AND LEFT.unit_desig=RIGHT.unit_desig AND LEFT.p_city_name=RIGHT.p_city_name AND LEFT.v_predir=RIGHT.v_predir AND LEFT.v_prim_name=RIGHT.v_prim_name AND LEFT.v_suffix=RIGHT.v_suffix AND LEFT.v_postdir=RIGHT.v_postdir AND LEFT.v_city_name=RIGHT.v_city_name AND LEFT.z4=RIGHT.z4 AND LEFT.county_code=RIGHT.county_code
              AND LEFT.geo_lat=RIGHT.geo_lat AND LEFT.geo_long=RIGHT.geo_long AND LEFT.msa=RIGHT.msa AND LEFT.designation=RIGHT.designation AND LEFT.name_prefix=RIGHT.name_prefix AND LEFT.name_first=RIGHT.name_first AND LEFT.name_middle=RIGHT.name_middle AND LEFT.name_last=RIGHT.name_last AND LEFT.name_suffix=RIGHT.name_suffix AND LEFT.listed_name=RIGHT.listed_name
              AND LEFT.caption_text=RIGHT.caption_text AND LEFT.omit_address=RIGHT.omit_address AND LEFT.omit_phone=RIGHT.omit_phone AND LEFT.omit_locality=RIGHT.omit_locality AND LEFT.see_also_text=RIGHT.see_also_text AND LEFT.did=RIGHT.did AND LEFT.hhid=RIGHT.hhid AND LEFT.bdid=RIGHT.bdid AND LEFT.dt_first_seen=RIGHT.dt_first_seen AND LEFT.dt_last_seen=RIGHT.dt_last_seen
              AND LEFT.current_record_flag=RIGHT.current_record_flag AND LEFT.deletion_date=RIGHT.deletion_date AND LEFT.disc_cnt6=RIGHT.disc_cnt6 AND LEFT.disc_cnt12=RIGHT.disc_cnt12 AND LEFT.disc_cnt18=RIGHT.disc_cnt18 AND LEFT.persistent_record_id=RIGHT.persistent_record_id AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_gong_history_fcra_address::Ingest_Cache',EXPIRE(I_gong_history_fcra_address.Config.PersistExpire));

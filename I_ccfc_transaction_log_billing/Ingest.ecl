...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.billing_report_code=RIGHT.billing_report_code AND LEFT.account_type=RIGHT.account_type AND LEFT.account_base_suffix=RIGHT.account_base_suffix AND LEFT.reference_number=RIGHT.reference_number
              AND LEFT.customer_number=RIGHT.customer_number AND LEFT.is_active=RIGHT.is_active AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.billing_report_code=RIGHT.billing_report_code AND LEFT.account_type=RIGHT.account_type AND LEFT.account_base_suffix=RIGHT.account_base_suffix AND LEFT.reference_number=RIGHT.reference_number
              AND LEFT.customer_number=RIGHT.customer_number AND LEFT.is_active=RIGHT.is_active AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_CCFC_transaction_log_billing::Ingest_Cache',EXPIRE(I_CCFC_transaction_log_billing.Config.PersistExpire));

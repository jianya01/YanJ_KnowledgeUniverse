...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.score_model=RIGHT.score_model AND LEFT.score_model_version=RIGHT.score_model_version AND LEFT.report_code=RIGHT.report_code AND LEFT.processing_status=RIGHT.processing_status
              AND LEFT.score=RIGHT.score AND LEFT.reasoncode_01=RIGHT.reasoncode_01 AND LEFT.reasoncode_01_text=RIGHT.reasoncode_01_text AND LEFT.reasoncode_02=RIGHT.reasoncode_02 AND LEFT.reasoncode_02_text=RIGHT.reasoncode_02_text AND LEFT.reasoncode_03=RIGHT.reasoncode_03 AND LEFT.reasoncode_03_text=RIGHT.reasoncode_03_text AND LEFT.reasoncode_04=RIGHT.reasoncode_04 AND LEFT.reasoncode_04_text=RIGHT.reasoncode_04_text AND LEFT.date_added=RIGHT.date_added
              AND LEFT.customer_input_flg=RIGHT.customer_input_flg AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.sequence=RIGHT.sequence AND LEFT.score_model=RIGHT.score_model AND LEFT.score_model_version=RIGHT.score_model_version AND LEFT.report_code=RIGHT.report_code AND LEFT.processing_status=RIGHT.processing_status
              AND LEFT.score=RIGHT.score AND LEFT.reasoncode_01=RIGHT.reasoncode_01 AND LEFT.reasoncode_01_text=RIGHT.reasoncode_01_text AND LEFT.reasoncode_02=RIGHT.reasoncode_02 AND LEFT.reasoncode_02_text=RIGHT.reasoncode_02_text AND LEFT.reasoncode_03=RIGHT.reasoncode_03 AND LEFT.reasoncode_03_text=RIGHT.reasoncode_03_text AND LEFT.reasoncode_04=RIGHT.reasoncode_04 AND LEFT.reasoncode_04_text=RIGHT.reasoncode_04_text AND LEFT.date_added=RIGHT.date_added
              AND LEFT.customer_input_flg=RIGHT.customer_input_flg AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_nonfcra_transaction_log_model::Ingest_Cache',EXPIRE(I_mbsi_nonfcra_transaction_log_model.Config.PersistExpire));

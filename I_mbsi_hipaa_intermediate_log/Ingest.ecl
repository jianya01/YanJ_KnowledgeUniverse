...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.process_type=RIGHT.process_type AND LEFT.processing_time=RIGHT.processing_time AND LEFT.source_code=RIGHT.source_code
              AND LEFT.content_type=RIGHT.content_type AND LEFT.version=RIGHT.version AND LEFT.reference_number=RIGHT.reference_number AND LEFT.content_data=RIGHT.content_data AND LEFT.processing_status=RIGHT.processing_status AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.product_id=RIGHT.product_id AND LEFT.date_added=RIGHT.date_added AND LEFT.process_type=RIGHT.process_type AND LEFT.processing_time=RIGHT.processing_time AND LEFT.source_code=RIGHT.source_code
              AND LEFT.content_type=RIGHT.content_type AND LEFT.version=RIGHT.version AND LEFT.reference_number=RIGHT.reference_number AND LEFT.content_data=RIGHT.content_data AND LEFT.processing_status=RIGHT.processing_status AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_hipaa_intermediate_log::Ingest_Cache',EXPIRE(I_mbsi_hipaa_intermediate_log.Config.PersistExpire));

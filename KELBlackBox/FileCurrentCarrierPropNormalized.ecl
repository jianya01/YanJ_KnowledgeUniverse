...
EXPORT FileCurrentCarrierPropNormalized := MODULE
...
// Cleaning Functions
 t_subjectidsetccstandaloneresultscust cleanSubjectIDSection(RECORDOF(KELBlackBox.FileCurrentCarrierProperty.subjectsearchinformationsection.subjectidsets) le, UNSIGNED8 recIdentifier, UNSIGNED8 cntr) := TRANSFORM
...
t_personaldatasectionccstandaloneresultscust cleanPersonalDataSections(RECORDOF(KELBlackBox.FileCurrentCarrierProperty.attachmentsection.personaldatasections) le, UNSIGNED8 recIdentifier, UNSIGNED8 cntr) := TRANSFORM
		SELF.RecordIdentifier := recIdentifier;
...
EXPORT FileCCPropReportRecap := PROJECT(FileCCPropertyRoot, 
																	TRANSFORM({INTEGER8 RecordIdentifier, RECORDOF(FileCCPropertyRoot.recapprocessingsection) - subjectlevelinquiryrecap}, 
																		SELF.RecordIdentifier := Left.RecordIdentifier; 
																		SELF := LEFT.recapprocessingsection));

//  General Messages - reportsectionheader																				
EXPORT FileCCPropGenMesg_ReportSection := PROJECT(FileCCPropertyRoot, 
																	TRANSFORM({INTEGER8 RecordIdentifier, RECORDOF(FileCCPropertyRoot.generalmessagessection) - messages}, 
																		SELF.RecordIdentifier := Left.RecordIdentifier; 
																		SELF := LEFT.generalmessagessection));																			

//  AttachmentSection - reportsectionheader		
EXPORT FileCCPropAttachSect_ReportSection := PROJECT(FileCCPropertyRoot, 
																	TRANSFORM({INTEGER8 RecordIdentifier, RECORDOF(FileCCPropertyRoot.attachmentsection) - personaldatasections - vehicledatasets}, 
																		SELF.RecordIdentifier := Left.RecordIdentifier; 
																		SELF := LEFT.attachmentsection));	

//  subjectSearchInformationSection - reportsectionheader
EXPORT FileCCPropSubjectSearch_ReportSection := PROJECT(FileCCPropertyRoot, 
																	TRANSFORM({INTEGER8 RecordIdentifier, RECORDOF(FileCCPropertyRoot.subjectsearchinformationsection) - subjectidsets}, 
																		SELF.RecordIdentifier := Left.RecordIdentifier; 
...
EXPORT FileCCPropSubjectRecap := normalize(FileCCPropertyRoot, left.recapprocessingsection.subjectlevelinquiryrecap, getRecapMessages(LEFT, Right, counter));
...
EXPORT FileCCPropGeneralMessages:= normalize(FileCCPropertyRoot, left.generalmessagessection.messages, getGenMessages(LEFT, Right, counter));
 


// **3** subjectSearchInformationSection - SubjectSearchId Sets
t_subjectidsetccstandaloneresultscust getSubIDsets(CC_Property_Response L, t_subjectidsetccstandaloneresultscust R, integer cntr) := transform
               self.RecordIdentifier := L.RecordIdentifier;
               self.SubjectIDRecordCounter := cntr;
               Self := R;
   						END;
...
EXPORT FileCCPropSubIDSets:= normalize(FileCCPropertyRoot, left.subjectsearchinformationsection.subjectidsets, getSubIDsets(LEFT, Right, counter));
// EXPORT FileCCPropSubIDSets2:= NORMALIZE(FileCCPropertyRoot, LEFT.subjectsearchinformationsection.subjectidsets, TRANSFORM(t_subjectidsetccstandaloneresultscust, SELF := RIGHT));
...
EXPORT FileCCPropAdditionalMesg:= normalize(FileCCPropertyRoot, left.additionalinformationsection.DisclaimAndPoliciesSet.messages, getAddMessages(LEFT, Right, counter));
 
// **5** attachmentsection			- personrecord
t_personaldatasectionccstandaloneresultscust getAttSectMessages(CC_Property_Response L, t_personaldatasectionccstandaloneresultscust R, integer cntr) := transform
            self.RecordIdentifier := L.RecordIdentifier;
            self.PersonRecordCounter := cntr;
            Self := R;
   				 END;

EXPORT FileCCPropAttachmentSect:= normalize(FileCCPropertyRoot, left.attachmentsection.PersonalDataSections, getAttSectMessages(LEFT, Right, counter));
// EXPORT FileCCPropAttachmentSect2:= NORMALIZE(FileCCPropertyRoot, LEFT.attachmentsection.PersonalDataSections, TRANSFORM(t_personaldatasectionccstandaloneresultscust, SELF := RIGHT));

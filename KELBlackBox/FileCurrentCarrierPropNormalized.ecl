...
EXPORT FileCCPropSubjectRecap := normalize(KELBlackBox.FileCurrentCarrierProperty, left.recapprocessingsection.subjectlevelinquiryrecap, getRecapMessages(LEFT, Right, counter));
...
EXPORT FileCCPropGeneralMessages:= normalize(KELBlackBox.FileCurrentCarrierProperty, left.generalmessagessection.messages, getGenMessages(LEFT, Right, counter));
...
EXPORT FileCCPropSubIDSets:= normalize(KELBlackBox.FileCurrentCarrierProperty, left.subjectsearchinformationsection.subjectidsets, getSubIDsets(LEFT, Right, counter));
...
EXPORT FileCCPropAdditionalMesg:= normalize(KELBlackBox.FileCurrentCarrierProperty, left.additionalinformationsection.DisclaimAndPoliciesSet.messages, getAddMessages(LEFT, Right, counter));

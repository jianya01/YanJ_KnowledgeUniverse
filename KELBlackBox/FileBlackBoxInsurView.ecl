...
EXPORT FileBlackBoxInsurView := IF(COUNT(_Control.LexIDFilterSet) <= 0, 
	...
	JOIN(DISTRIBUTE(DATASET(fileName, LayoutInsurView, THOR, __COMPRESSED__), HASH64((UNSIGNED8)LexID)), DISTRIBUTE(_Control.LexIDFilterSet(LexID > 0), HASH64(LexID)), (UNSIGNED8)LEFT.LexID = RIGHT.LexID, TRANSFORM(LEFT), LOCAL));
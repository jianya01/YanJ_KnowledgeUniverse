...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.type=RIGHT.type AND LEFT.request_data=RIGHT.request_data AND LEFT.response_data=RIGHT.response_data AND LEFT.request_format=RIGHT.request_format
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.batch_job_id=RIGHT.batch_job_id AND LEFT.batch_seq_number=RIGHT.batch_seq_number AND LEFT.type=RIGHT.type AND LEFT.request_data=RIGHT.request_data AND LEFT.response_data=RIGHT.response_data AND LEFT.request_format=RIGHT.request_format
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_mbsi_hipaa_transaction_log_batch::Ingest_Cache',EXPIRE(I_mbsi_hipaa_transaction_log_batch.Config.PersistExpire));

...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.agency_type=RIGHT.agency_type AND LEFT.order_id=RIGHT.order_id AND LEFT.claim_number=RIGHT.claim_number AND LEFT.claim_state=RIGHT.claim_state AND LEFT.date_loss=RIGHT.date_loss
              AND LEFT.report_type=RIGHT.report_type AND LEFT.policy_number=RIGHT.policy_number AND LEFT.return_options=RIGHT.return_options AND LEFT.order_type=RIGHT.order_type AND LEFT.initial_status=RIGHT.initial_status AND LEFT.date_returned=RIGHT.date_returned AND LEFT.data_elements=RIGHT.data_elements AND LEFT.return=RIGHT.return AND LEFT.report_available=RIGHT.report_available AND LEFT.date_added=RIGHT.date_added
              AND LEFT.agency_name=RIGHT.agency_name AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.agency_type=RIGHT.agency_type AND LEFT.order_id=RIGHT.order_id AND LEFT.claim_number=RIGHT.claim_number AND LEFT.claim_state=RIGHT.claim_state AND LEFT.date_loss=RIGHT.date_loss
              AND LEFT.report_type=RIGHT.report_type AND LEFT.policy_number=RIGHT.policy_number AND LEFT.return_options=RIGHT.return_options AND LEFT.order_type=RIGHT.order_type AND LEFT.initial_status=RIGHT.initial_status AND LEFT.date_returned=RIGHT.date_returned AND LEFT.data_elements=RIGHT.data_elements AND LEFT.return=RIGHT.return AND LEFT.report_available=RIGHT.report_available AND LEFT.date_added=RIGHT.date_added
              AND LEFT.agency_name=RIGHT.agency_name AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_rules_transaction_log_police::Ingest_Cache',EXPIRE(I_rules_transaction_log_police.Config.PersistExpire));

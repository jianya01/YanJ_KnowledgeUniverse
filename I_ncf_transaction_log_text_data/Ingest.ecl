...
, DATASET(RECORDOF(Source))  infile = Source
  ...
  Ingest0 := JOIN( Base0,FilesToIngest0,LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.score_model_version_list=RIGHT.score_model_version_list AND LEFT.individual_scores_and_reason_codes_list=RIGHT.individual_scores_and_reason_codes_list AND LEFT.spouse_scores_and_reason_codes_list=RIGHT.spouse_scores_and_reason_codes_list AND LEFT.individual_alert_messages_list=RIGHT.individual_alert_messages_list AND LEFT.spouse_alert_messages_list=RIGHT.spouse_alert_messages_list
              AND LEFT.opr_list=RIGHT.opr_list AND LEFT.condition_code_text=RIGHT.condition_code_text AND LEFT.score_model_list_billing=RIGHT.score_model_list_billing AND LEFT.user_added=RIGHT.user_added AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_first_seen=RIGHT.vault_date_first_seen AND LEFT.vault_date_last_seen=RIGHT.vault_date_last_seen,MergeData(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  Ingest1 := JOIN( Ingest0(__Tpe=RecordType.Old),Ingest0(__Tpe=RecordType.New),LEFT.vault_uid_hash=RIGHT.vault_uid_hash AND LEFT.transaction_id=RIGHT.transaction_id AND LEFT.score_model_version_list=RIGHT.score_model_version_list AND LEFT.individual_scores_and_reason_codes_list=RIGHT.individual_scores_and_reason_codes_list AND LEFT.spouse_scores_and_reason_codes_list=RIGHT.spouse_scores_and_reason_codes_list AND LEFT.individual_alert_messages_list=RIGHT.individual_alert_messages_list AND LEFT.spouse_alert_messages_list=RIGHT.spouse_alert_messages_list
              AND LEFT.opr_list=RIGHT.opr_list AND LEFT.condition_code_text=RIGHT.condition_code_text AND LEFT.score_model_list_billing=RIGHT.score_model_list_billing AND LEFT.user_added=RIGHT.user_added AND LEFT.date_added=RIGHT.date_added AND LEFT.vault_date_last_seen=0 AND RIGHT.vault_date_last_seen=0,MergeData1(LEFT,RIGHT),FULL OUTER,HASH);
  ...
  SHARED AllRecs := ORe+NR1+NR(vault_rid<>0) : PERSIST('~temp::I_ncf_transaction_log_text_data::Ingest_Cache',EXPIRE(I_ncf_transaction_log_text_data.Config.PersistExpire));
